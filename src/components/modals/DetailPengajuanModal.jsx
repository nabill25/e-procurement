        ...
        <div className="space-y-1">
          {files.map(f => (
            <div key={f.id} className="flex items-center justify-between text-xs bg-surface p-2 rounded-lg">
              ...
              <a href={resolveFileUrl(f.file_path)} target="_blank" rel="noreferrer" className="text-blue-600"><Download size={12} /></a>
            </div>
          ))}
        </div>
      )}
        ...
        {revisions.map(r => (
          <div key={r.id} className="bg-red-50 border border-red-200 rounded-lg p-3 text-xs">
            ...
            <div className="flex items-center justify-between mt-1.5">
              ...
              {r.file_path && <a href={resolveFileUrl(r.file_path)} target="_blank" rel="noreferrer" className="text-blue-600"><Download size={12} /></a>}
            </div>
          </div>
        ))}
      </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={doc.label} className="border border-border rounded-xl p-3 flex flex-col items-center justify-center text-center bg-gray-50 hover:bg-gray-100 transition-colors">
                  ...
                  {doc.path ? (
                    <a href={resolveFileUrl(doc.path)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[10px] text-blue-600 font-semibold bg-blue-50 px-2 py-1 rounded">
