            ...
            {documents.map(d => (
              <div key={d.id} className="flex items-center justify-between text-xs bg-surface p-2 rounded-lg">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <a href={resolveFileUrl(d.file_path)} target="_blank" rel="noreferrer" className="text-blue-600"><Download size={13} /></a>
            ...
            {klarifikasi.filter(k => !k.parent_id).map(k => (
              <div key={k.id} className="bg-surface p-2.5 rounded-lg">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <a href={resolveFileUrl(k.file_path)} target="_blank" rel="noreferrer" className="text-blue-600 shrink-0"><Download size={13} /></a>
                </div>
                {klarifikasi.filter(t => t.parent_id === k.id).map(t => (
                  <div key={t.id} className="mt-1.5 ml-3 pl-2 border-l-2 border-dpbj-gold/40 flex items-center justify-between text-[11px]">
                    ...
                    <a href={resolveFileUrl(t.file_path)} target="_blank" rel="noreferrer" className="text-blue-600"><Download size={12} /></a>
                  </div>
                ))}
