...
if (!empty($_SERVER['HTTP_HOST'])) {
    $eproc_self_url = "http://".$_SERVER['HTTP_HOST'];
    $eproc_self_url .= str_replace(basename($_SERVER['SCRIPT_NAME']), "", $_SERVER['SCRIPT_NAME']);
    $eproc_self_url = rtrim($eproc_self_url, '/');
} else {
    $eproc_self_url = 'http://10.39.28.110'; // fallback kalau dijalankan lewat CLI/cron, perilaku lama, tidak diubah
}
define('SYSTEM_NAME_URL', $eproc_self_url);
define('SYSTEM_URL_EMAIL', $eproc_self_url);
...
// Tujuan email "penyedia sudah melengkapi dokumen" (fitur Tindak Lanjut
// Kelengkapan, 2026-09-01). Di DB tidak ada kolom email per akun verifikator
// internal, jadi notifikasi diarahkan ke satu inbox tim verifikasi penyedia.
// ISI dengan alamat email tim yang benar sebelum fitur dipakai sungguhan.
define('EMAIL_TINDAK_LANJUT_FALLBACK', '');
define('SYSTEM_LOGO', 'logo.png');
define('SYSTEM_LOGO_CETAK', 'logo.png'); // logo-cetak-copy.jpg
define('SYSTEM_LOGO_URL', $eproc_self_url.'/images/logo.png');
define('SYSTEM_LOGO_URL_WHITE', $eproc_self_url.'/images/logo.png');
define('SYSTEM_LOADING', $eproc_self_url.'/images/page-loader-old.gif');
