...
1. **`getAuthHeaders` salah dipanggil** di `LoginLogs.jsx` dan `ApiKeys.jsx` - ditulis `const { getAuthHeaders } = useApp()` padahal `getAuthHeaders` itu fungsi yang di-export terpisah dari `AppContext.jsx` (bukan bagian dari `value` yang dikembalikan `useApp()`). Akibatnya kedua halaman baru ini diam-diam gagal (ketangkap try/catch, tidak sampai bikin layar putih, tapi datanya tidak pernah kebaca/tersimpan - riwayat login selalu kosong, API key tidak pernah tersimpan walau tombol submit ditekan). Sudah diperbaiki: import langsung `getAuthHeaders` dari `../context/AppContext`, bukan destructure dari `useApp()`.
   - **Catatan penting untuk nanti**: pola salah yang SAMA ternyata ada di 11 file LAIN yang sudah ada sebelumnya (`DataMaster.jsx`, `ContractTab.jsx`, `ContractWorkflowSections.jsx`, `DetailTenderModal.jsx`, `VendorProfile.jsx`, `FormulaCategorySection.jsx`, `UserManagement.jsx`, `ContractDetailSections.jsx`, `ContentManagement.jsx`, `MenuAccess.jsx`, `ObjectionsTab.jsx`). Ini **TIDAK diperbaiki di sesi ini** karena di luar cakupan kelompok G, tapi dicatat di sini karena kemungkinan menyebabkan bug diam-diam serupa di banyak fitur yang butuh header otentikasi di file-file itu. Perlu diperbaiki lain waktu (baik sekaligus atau saat masing-masing fitur itu disentuh lagi).
