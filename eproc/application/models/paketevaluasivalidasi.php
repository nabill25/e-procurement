 ...
 */
  ...
  class PaketEvaluasiValidasi extends Entity{ 

	var $query;
    /**
    * Class constructor.
    **/
    function PaketEvaluasiValidasi()
	{
      $this->Entity(); 
    }
	...
	function insert()
	{
		...
		$str = "INSERT INTO PAKET_EVALUASI_VALIDASI (
	...
	function deletePaktaRekanan()
	{
		...
		$str = "DELETE FROM PAKET_EVALUASI_VALIDASI 
				WHERE 
    ...
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = "SELECT PAKET_ID, USER_LOGIN_ID, KODE, JENIS, KODE_QR
				FROM PAKET_EVALUASI_VALIDASI WHERE PAKET_ID IS NOT NULL"; 
	...
	function selectByParamsValidasi($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = "SELECT A.PAKET_ID, NIP, NAMA, A.JENIS, KODE FROM 
                ...
                LEFT JOIN PAKET_EVALUASI_VALIDASI B ON A.PAKET_ID = B.PAKET_ID AND A.NIP = B.KODE
                 WHERE 1 = 1"; 
	...
	function selectByParamsValidasiPublish($paket_id)
	{
		$str = "SELECT (SELECT COUNT(1) FROM PAKET_ANGGOTA WHERE PAKET_ID = ".$paket_id." AND JENIS IN ('PANITIA', 'FUNGSIONAL')) TOTAL_ANGGOTA,
				COALESCE((SELECT COUNT(1) FROM PAKET_ANGGOTA A INNER JOIN PAKET_EVALUASI_VALIDASI B ON A.PAKET_ID = B.PAKET_ID AND A.NIP = B.KODE WHERE A.PAKET_ID = ".$paket_id." AND A.JENIS IN ('PANITIA', 'FUNGSIONAL')), 0) TOTAL_TTD
				 FROM DUAL
			  "; 				
    ...
    **/ 
	...
	{
		$str = "SELECT COUNT(PAKET_ID) AS ROWCOUNT FROM PAKET_EVALUASI_VALIDASI A WHERE PAKET_ID IS NOT NULL "; 
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key = '$val' ";
		}
