 ...
 */
  ...
  function insertUndang2Bypass()
	{
		...
		$str = "
				INSERT INTO PAKET_REKANAN (
				   PAKET_REKANAN_ID, PAKET_ID, REKANAN_ID,
				   TANGGAL_UNDANG, TANGGAL_DAFTAR, DI_EMAIL,LULUS_PENDAFTARAN,LULUS_KUALIFIKASI,KODE_REKANAN,LULUS_PENAWARAN,LULUS_ADMINISTRASI,LULUS_PENAWARAN_KETERANGAN,KIRIM_PENAWARAN_KODE)
				VALUES (".$this->getField("PAKET_REKANAN_ID").", ".$this->getField("PAKET_ID").", ".$this->getField("REKANAN_ID").",
				   NOW(), NOW(), ".$this->getField("DI_EMAIL").", ".$this->getField("LULUS_PENDAFTARAN").", ".$this->getField("LULUS_KUALIFIKASI").", '".$this->getField("KODE_REKANAN")."' || LPAD('".$this->getField("PAKET_REKANAN_ID")."', '6', '0'), ".$this->getField("LULUS_PENAWARAN").", ".$this->getField("LULUS_ADMINISTRASI").", '".$this->getField("LULUS_PENAWARAN_KETERANGAN")."', '".$this->getField("KIRIM_PENAWARAN_KODE")."')
			";
		// echo $str; die();
		$this->query = $str;
		$this->paket_rekanan_id = $this->getField("PAKET_REKANAN_ID");
		if ($this->execQuery($str)) {
			$str2 = "
				INSERT INTO REKANAN_PAKET_PENAWARAN (
				   REKANAN_PAKET_PENAWARAN_ID, PAKET_PENAWARAN_ID, PAKET_REKANAN_ID, QUANTITY, LAST_CREATE_USER,
				   LAST_CREATE_DATE)
				VALUES (".$this->getField("REKANAN_PAKET_PENAWARAN_ID").",".$this->getField("PAKET_PENAWARAN_ID").", $this->paket_rekanan_id, 1, ".$this->getField("LAST_CREATE_USER").",NOW())
			";
			if ($this->execQuery($str2)) {
				return true;
			} else {
				$del = "DELETE FROM PAKET_REKANAN WHERE PAKET_REKANAN_ID = ".$this->paket_rekanan_id." ";
				$this->execQuery($del);
				return false;
			}
		} else {
  ...
  function selectByParamsPaketPemenang($paramsArray=array(),$limit=-1,$from=-1, $statement='')
    ...
    **/
    ...
    function updateNegoBypass()
	{

		$str = "UPDATE PAKET_REKANAN  SET
					NILAI_PENAWARAN = ".$this->getField("NILAI_PENAWARAN")."
					WHERE PAKET_ID = ".$this->getField("PAKET_ID")." AND PAKET_REKANAN_ID = ".$this->getField("PAKET_REKANAN_ID")." ";
		$this->query = $str;
		if($this->execQuery($str)) {
			$str2 = "UPDATE REKANAN_PAKET_PENAWARAN  SET
					UNIT_PRICE = ".$this->getField("NILAI_PENAWARAN").", 
					JUMLAH = ".$this->getField("NILAI_PENAWARAN")."
					WHERE PAKET_PENAWARAN_ID = ".$this->getField("PAKET_PENAWARAN_ID")." AND PAKET_REKANAN_ID = ".$this->getField("PAKET_REKANAN_ID")." ";
			$this->query = $str2;
			if($this->execQuery($str2)) {
				return true;
			} else {
    ...
    function insertNegoBypass()
	{ 
		$this->setField("PAKET_NEGOSIASI_ID", $this->getNextId("PAKET_NEGOSIASI_ID","PAKET_NEGOSIASI"));
		$str = "
				INSERT INTO PAKET_NEGOSIASI (
				   PAKET_NEGOSIASI_ID, PAKET_ID, JUMLAH, PAKET_PENAWARAN_ID, UNIT_PRICE, QUANTITY, UNIT_PRICE_AWAL,
				   CREATED_BY, CREATED_DATE)
				VALUES (".$this->getField("PAKET_NEGOSIASI_ID").", ".$this->getField("PAKET_ID").", ".$this->getField("NILAI_NEGOSIASI").",
				   ".$this->getField("PAKET_PENAWARAN_ID").", ".$this->getField("NILAI_NEGOSIASI").", '1',
				   ".$this->getField("NILAI_NEGOSIASI").", ".$this->getField("CREATED_BY").", CURRENT_TIMESTAMP)
			";

		$this->query = $str;

			if($this->execQuery($str)) {
				$str2 = "UPDATE PAKET  SET
					REKANAN_ID_PEMENANG = ".$this->getField("REKANAN_ID")."
					WHERE PAKET_ID = ".$this->getField("PAKET_ID")." ";
				$this->query = $str2;
				if($this->execQuery($str2)) {
					return true;
    ...
    function update2NegoBypass()
	{ 
		$str = "
				UPDATE PAKET_NEGOSIASI SET  
					JUMLAH = ".$this->getField("NILAI_NEGOSIASI").",
					UNIT_PRICE = ".$this->getField("NILAI_NEGOSIASI").",
					UNIT_PRICE_AWAL = ".$this->getField("NILAI_NEGOSIASI")."
					WHERE PAKET_ID = ".$this->getField("PAKET_ID")." AND PAKET_PENAWARAN_ID = ".$this->getField("PAKET_PENAWARAN_ID")." ";
