  ...
  function selectByParamsMonitoring2($paramsArray=array(),$limit=-1,$from=-1, $statement='')
		...
		if ($tahun == '' || $tahun == 'all') {
			...
			$str = "SELECT A.* FROM (
					SELECT x.paket_id, z.user_login_id, h.direktorat_id, g.nama nama_direktorat, h.department,z.permohonan_paket_id, z.nama, z.nilai, z.perkiraan_biaya_harga, n.harga_negosiasi, 
						...
						WHERE A.nama_direktorat = '".$nama_direktorat."' AND A.tanggal_permohoanan = '".$tahun."'
					";
		}
		// echo $str;
		$this->select($str);
	...
	function getDashboardGauge($unitkerja,$vppengadaan,$tahun=null)
	{
		if ($tahun == '' || $tahun == 'all') {
			...
			if ($unitkerja == 'all') {
				$str = "SELECT
						(select count(paket_id) total_paket from view_dashboard_paket_proses where vp_pengadaan = '".$vppengadaan."' and tahun_permohonan='".$tahun."'),
						(select count(paket_id) total_paket_proses from view_dashboard_paket_proses where vp_pengadaan = '".$vppengadaan."' and proses = '1' and tahun_permohonan='".$tahun."')
						";
			} else {
				$str = "SELECT
						(select count(paket_id) total_paket from view_dashboard_paket_proses where vp_pengadaan = '".$vppengadaan."' and tahun_permohonan='".$tahun."' and unit_kerja_id = '".$unitkerja."'),
						(select count(paket_id) total_paket_proses from view_dashboard_paket_proses where vp_pengadaan = '".$vppengadaan."' and proses = '1' and tahun_permohonan='".$tahun."' and unit_kerja_id = '".$unitkerja."')
						";
			}
		}
		// echo $str;
...
//                $this->query = $str;
	...
	{
		$str = "SELECT A.* FROM (
					SELECT A.*, D.VP_PENGADAAN, date_part('year'::text, A.tanggal) AS TAHUN 
					FROM PAKET A
					LEFT JOIN PERMOHONAN_PAKET B ON A.PERMOHONAN_PAKET_ID = B.PERMOHONAN_PAKET_ID
    ...
    function selectKontrakProses($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = " SELECT A.*, 
				(SELECT SUM(Z.CR_NILAI_KONTRAK) FROM CONTRACTING_REKANAN_PROSES1 Z WHERE A.CONTRACTINGREKANANID=Z.CONTRACTINGREKANANID ) AS CR_NILAI_KONTRAK FROM (
						SELECT A.NAMA, D.VP_PENGADAAN, date_part('year'::text, A.tanggal) AS TAHUN, B.CONTRACTINGREKANANID
						FROM PAKET A
						JOIN CONTRACTING_REKANAN B ON A.PAKET_ID=B.PAKET_ID
		...
		$str .= $statement.' GROUP BY A.NAMA, A.TAHUN, A.CONTRACTINGREKANANID, A.VP_PENGADAAN ';
		// echo $str; die();
		$this->query = $str;
		return $this->selectLimit($str,$limit,$from);
  	}
  	...
  	function selectKontrakProsesSum($paramsArray=array(), $statement='')
	{
		$str = "SELECT SUM(A.NILAI_KONTRAK) AS ROWCOUNT
                FROM ( 
	                SELECT A.* FROM (
						SELECT A.PAKET_ID, A.NAMA, D.VP_PENGADAAN, date_part('year'::text, A.tanggal) AS TAHUN, B.CONTRACTINGREKANANID, SUM(C.CR_NILAI_KONTRAK) NILAI_KONTRAK, SUM(C.CR_SPPBJ_NILAI) NILAI_SPPBJ
						FROM PAKET A
						JOIN CONTRACTING_REKANAN B ON A.PAKET_ID=B.PAKET_ID
						LEFT JOIN CONTRACTING_REKANAN_PROSES1 C ON B.CONTRACTINGREKANANID=C.CONTRACTINGREKANANID
						LEFT JOIN PERMOHONAN_PAKET BB ON A.PERMOHONAN_PAKET_ID = BB.PERMOHONAN_PAKET_ID
						...
						WHERE A.NAMA IS NOT NULL 
						GROUP BY A.PAKET_ID, A.NAMA, B.CONTRACTINGREKANANID, D.VP_PENGADAAN
					) A 
					WHERE 1=1
					";
		while(list($key,$val)=each($paramsArray))
