 ...
 function updatepr()
  ...
  function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='',  $order="ORDER BY ID ASC")
  {
    $str = "SELECT A.* FROM (
              ...
              A.METODE_PEMILIHAN, A.NAMA_JENIS_PEKERJAAN, A.HASIL_VERIFIKASI, A.IMPORT_DATE, A.IMPORT_FILE, B.PERMOHONAN_PAKET_ID, B.UNIT_KERJA_ID, C.PERMOHONAN_PAKET_ANALISA_ID, A.KODE_PR, B.KODE_PR KODE_PR_PERMOHONAN, A.KODE_PR KODE_PR_RUP, A.NILAI_PAGU_PR, A.SUMBER_DANA
              FROM IMPORT_SIRUP A
	...
	function getCountByParams($paramsArray=array(), $statement='')
  {
    $str = "SELECT COUNT(A.ID) AS ROWCOUNT FROM (
                ...
                SELECT A.ID, A.TAHUN, A.KODE_RUP, A.KODE_SA, A.KODE_DPSJ, A.NO_URUT, A.KATEGORI_PAKET_ID, A.NAMA_PAKET, A.NILAI_PAGU, A.LIST_KEGIATAN, A.WAKTU_AWAL,
                      A.WAKTU_AKHIR, A.STATUS_PROSES, A.CREATED_BY, A.CREATED_AT, A.UPDATED_BY, A.UPDATED_AT, A.NAME, A.KATEGORI_PAKET, A.NAMA_SA, A.NAMA_DPSJ,
                      A.METODE_PEMILIHAN, A.NAMA_JENIS_PEKERJAAN, A.HASIL_VERIFIKASI, A.IMPORT_DATE, A.IMPORT_FILE, B.PERMOHONAN_PAKET_ID, B.UNIT_KERJA_ID, C.PERMOHONAN_PAKET_ANALISA_ID
                      FROM IMPORT_SIRUP A
                      LEFT JOIN PERMOHONAN_PAKET B ON A.ID=B.SIRUP_ID
                      LEFT JOIN PERMOHONAN_PAKET_ANALISA C ON B.PERMOHONAN_PAKET_ANALISA_ID=C.PERMOHONAN_PAKET_ANALISA_ID
                ) A where 1=1 ";
  ...
  function selectByParamsPersiapan($paramsArray=array(),$limit=-1,$from=-1, $statement='',  $order="ORDER BY PERMOHONAN_PAKET_ID ASC")
	{
		$str = "SELECT A.* FROM (
              SELECT A.PERMOHONAN_PAKET_ID, A.USER_LOGIN_ID, A.UNIT_KERJA_ID, A.NAMA, A.LAST_CREATE_USER, A.LAST_CREATE_DATE, A.NILAI, A.APPROVAL APPROVAL_PERMOHONAN_PAKET,A.PERMOHONAN_PAKET_ID_PARENT,
