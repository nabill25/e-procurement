 ...
 */
  ...
  class Paketpemenang extends Entity{ 
    ...
    **/
	...
	function insert()
	{
		...
		$this->setField("PAKET_PEMENANG_ID", $this->getNextId("PAKET_PEMENANG_ID","PAKET_PEMENANG")); 

		$str = "
			INSERT INTO PAKET_PEMENANG (
			   PAKET_PEMENANG_ID, REKANAN_ID, KETERANGAN, 
	...
	function delete()
	{
        $str = "DELETE FROM PAKET_PEMENANG
                WHERE 
    ...
    **/ 
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = "
					SELECT 
					A.PAKET_PEMENANG_ID,A.KETERANGAN, A.TANGGAL_PENETAPAN, A.PAKET_ID, A.FILE, A.PUBLISH, B.REKANAN_ID, C.NAMA || ' ' || B.NAMA NAMA, B.NPWP, B.ALAMAT, B.TELEPON,
					B.FAX, B.EMAIL, B.KONTAK_PERSON, B.KONTAK_PERSON_HP, B.WEBSITE, A.PERINGKAT
					FROM PAKET_PEMENANG A
	...
	{
		$str = "SELECT 
					COUNT(A.PAKET_PEMENANG_ID) AS ROWCOUNT
					FROM PAKET_PEMENANG A
					LEFT JOIN REKANAN B ON A.REKANAN_ID = B.REKANAN_ID
				    WHERE A.PAKET_ID IS NOT NULL  
			   ".$statement;
	...
	{
		/*Auto-generate primary key(s) by next max value (integer) */
		$str = "UPDATE PAKET_PEMENANG 
				SET
					PUBLISH = '".$this->getField("PUBLISH")."'
				WHERE PAKET_PEMENANG_ID = '".$this->getField("PAKET_PEMENANG_ID")."'
				"; 
				$this->query = $str;
		return $this->execQuery($str);
    }
