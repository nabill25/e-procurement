...
class Format {
    ...
    public function __construct($data = NULL, $from_type = NULL){
        ...
        if ($from_type !== NULL)
        {
            ...
            {
                $data = call_user_func([$this, 'from_' . $from_type], $data);
            }
            else
            {
     ...
     */
    protected function from_xml($data)
    {
        if($this->valid_XML($data))
     ...
     */
    protected function from_json($data)
    {
        $data=$this->valid_JSON($data);
        if($data!==FALSE)
            return $data;
        throw new Exception('INVALID INPUT JSON FORMAT');
    }
    /**
     * @param string Data to unserialized
     * @return mixed Unserialized data
     */
    protected function from_serialize($data)
    {
    ...
    /**
     * @param $args json input data
     * @return bool|mixed
     */
    function valid_JSON($args) {
     ...
     * @return bool
     */
    public function valid_XML($xmlContent, $version = '1.0', $encoding = 'utf-8')
    {
