 ...
 */
  ...
  class Dashpaket extends Entity{
  ...
  function selectByParamsMonitoring2($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	...
	function getDashboardBar2Detail($tahun=null,$nama_direktorat)
	{
		if ($tahun == '' || $tahun == 'all') {
			$str = "	SELECT A.* FROM (
						SELECT x.paket_id, z.user_login_id, h.direktorat_id, g.nama nama_direktorat,h.department,z.permohonan_paket_id, z.nama, z.nilai, z.perkiraan_biaya_harga, n.harga_negosiasi, 
							( 
	...
	function getDashboardBar2Detail2($tahun=null,$user_login_id,$unitkerja=null)
	{
		if ($tahun == '' || $tahun == 'all') {
			...
			if ($unitkerja == 'all') {
				$str = "SELECT A.* FROM (
						SELECT x.paket_id, z.user_login_id, z.unit_kerja_id, z.permohonan_paket_id, z.nama, z.nilai, extract(year from z.tanggal) tanggal_permohoanan, z.tahun_anggaran,
							(SELECT case when a.alasan_ulang != '' OR a.alasan != '' then '0' when n.harga_negosiasi::text is null OR n.harga_negosiasi <= 0 then '0' else '1' end total 
							from paket a 
							LEFT JOIN view_dash_efesiensi n ON a.paket_id = n.paket_id 
							where a.permohonan_paket_id=z.permohonan_paket_id
				...
				$str = "SELECT A.* FROM (
						SELECT x.paket_id, z.user_login_id, z.unit_kerja_id, z.permohonan_paket_id, z.nama, z.nilai, extract(year from z.tanggal) tanggal_permohoanan, z.tahun_anggaran,
							...
							WHERE A.user_login_id = '".$user_login_id."' AND A.tahun_anggaran = '".$tahun."' AND A.unit_kerja_id = '".$unitkerja."'
						";
			}
		}
		// echo $str;
	...
	function getDashboardBar2Detail3($tahun=null,$user_login_id)
	{
		if ($tahun == '' || $tahun == 'all') {
			$str = "	SELECT A.* FROM (
						SELECT x.paket_id, z.user_login_id, h.direktorat_id, g.nama nama_direktorat,z.permohonan_paket_id, z.nama, z.nilai, z.perkiraan_biaya_harga, n.harga_negosiasi, extract(year from z.rencana_pengadaan) tanggal_permohoanan, z.tahun_anggaran,
							(SELECT case when a.alasan_ulang != '' OR a.alasan != '' then '0' when n.harga_negosiasi::text is null OR n.harga_negosiasi <= 0 then '0' else '1' end total 
							from paket a 
							LEFT JOIN view_dash_efesiensi n ON a.paket_id = n.paket_id 
							where a.permohonan_paket_id=z.permohonan_paket_id
	...
	function getDashboardGaugeAll($unitkerja,$tahun=null)
	{
		if ($tahun == '' || $tahun == 'all') {
			if ($unitkerja == 'all') {
				$str = "SELECT
...
//		echo $str;
    ...
    function getCountByParams($paramsArray=array(), $statement='')
	{
		$str = "select COUNT(*) AS ROWCOUNT
			...
			A.PERMOHONAN_PAKET_ID, I.KODE_RUP, I.KODE_PR, A.PAKET_JENIS_ID
                    FROM    PAKET A
                            LEFT JOIN PAKET_METODE_LELANG B ON A.PAKET_METODE_LELANG_ID = B.PAKET_METODE_LELANG_ID
                            LEFT JOIN PAKET_METODE_KUALIFIKASI C ON A.PAKET_METODE_KUALIFIKASI_ID = C.PAKET_METODE_KUALIFIKASI_ID
                            LEFT JOIN PAKET_METODE_EVALUASI D ON A.PAKET_METODE_EVALUASI_ID = D.PAKET_METODE_EVALUASI_ID
    ...
    function getSumByParams($paramsArray=array(), $statement='')
	{
		$str = "SELECT SUM(NILAI) AS ROWCOUNT
                        from ( SELECT
                      A.NILAI, A.PAKET_METODE_LELANG_ID, A.TANGGAL, A.UNIT_KERJA_ID, A.USER_LOGIN_ID, date_part('month'::text, A.tanggal) AS bulan, date_part('month'::text, I.rencana_pengadaan) AS bulan_permohonan, date_part('year'::text, A.tanggal) AS TAHUN, date_part('year'::text, I.rencana_pengadaan) AS TAHUN_PERMOHONAN, I.TAHUN_ANGGARAN
                    FROM    PAKET A
                            LEFT JOIN PAKET_METODE_LELANG B ON A.PAKET_METODE_LELANG_ID = B.PAKET_METODE_LELANG_ID
                            LEFT JOIN PAKET_METODE_KUALIFIKASI C ON A.PAKET_METODE_KUALIFIKASI_ID = C.PAKET_METODE_KUALIFIKASI_ID
                            LEFT JOIN PAKET_METODE_EVALUASI D ON A.PAKET_METODE_EVALUASI_ID = D.PAKET_METODE_EVALUASI_ID
...
//                $this->query = $str;
	...
	{
		$str = "SELECT A.*, B.NAMA, B.PERKIRAAN_BIAYA_HARGA, B.KODE_RUP, B.KODE_PR, C.DEPARTMENT
  	...
  	function selectPermohonanSum($paramsArray=array(), $statement='')
	{
		$str = "SELECT SUM(A.PERKIRAAN_BIAYA_HARGA) AS ROWCOUNT
                FROM ( SELECT B.PERKIRAAN_BIAYA_HARGA FROM PERMOHONAN_PAKET_ANALISA A 
	...
	{  
		$str = "SELECT A.* FROM (
					SELECT A.*, date_part('year'::text, A.tanggal) AS TAHUN, date_part('year'::text, B.rencana_pengadaan) AS tahun_permohonan, B.tahun_anggaran, B.user_login_id pembuat, C.DEPARTMENT, D.NAMA METODE
					FROM PAKET A
					JOIN PERMOHONAN_PAKET B ON A.PERMOHONAN_PAKET_ID=B.PERMOHONAN_PAKET_ID
					JOIN USER_LOGIN C ON B.USER_LOGIN_ID=C.USER_LOGIN_ID
					JOIN PAKET_METODE_LELANG D ON A.PAKET_METODE_LELANG_ID=D.PAKET_METODE_LELANG_ID
					WHERE A.NAMA IS NOT NULL 
				) A WHERE 1=1 ";
  	...
  	function selectHargaFinalByPaketId($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{  
		$str = "SELECT A.* FROM (
					SELECT A.PAKET_ID, A.REKANAN_ID, D.NAMA || ' ' || C.NAMA NAMA, B.CR_NILAI_KONTRAK, B.CR_SPPBJ_NILAI 
					FROM CONTRACTING_REKANAN A 
						LEFT JOIN CONTRACTING_REKANAN_PROSES1 B ON A.CONTRACTINGREKANANID=B.CONTRACTINGREKANANID 
						JOIN REKANAN C ON A.REKANAN_ID=C.REKANAN_ID
  	...
  	function selectPemilihanSum($paramsArray=array(), $statement='')
	{
		$str = "SELECT SUM(A.NILAI) AS ROWCOUNT
                FROM ( 
                SELECT A.* FROM (
                	SELECT A.*, date_part('year'::text, A.tanggal) AS TAHUN, date_part('year'::text, B.rencana_pengadaan) AS tahun_permohonan, B.tahun_anggaran
                	FROM PAKET A
					JOIN PERMOHONAN_PAKET B ON A.PERMOHONAN_PAKET_ID=B.PERMOHONAN_PAKET_ID
					WHERE A.NAMA IS NOT NULL 
				) A WHERE 1=1";
	...
	{
		$str = "SELECT A.* FROM (
					SELECT A.*, date_part('year'::text, A.tanggal) AS TAHUN, B.tahun_anggaran, B.user_login_id pembuat
					FROM PAKET A
					LEFT JOIN permohonan_paket B ON A.permohonan_paket_id = B.permohonan_paket_id
					WHERE A.NAMA IS NOT NULL 
				) A WHERE 1=1 ";
    ...
    function selectKontrakProses($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = " SELECT A.*, 
				(SELECT SUM(Z.CR_NILAI_KONTRAK) FROM CONTRACTING_REKANAN_PROSES1 Z WHERE A.CONTRACTINGREKANANID=Z.CONTRACTINGREKANANID ) AS CR_NILAI_KONTRAK FROM (
						SELECT A.NAMA, date_part('year'::text, I.rencana_pengadaan) AS TAHUN_PERMOHONAN, I.TAHUN_ANGGARAN, B.CONTRACTINGREKANANID, C.DEPARTMENT, D.CP_NAME
						FROM PAKET A
						JOIN CONTRACTING_REKANAN B ON A.PAKET_ID=B.PAKET_ID
						LEFT JOIN PERMOHONAN_PAKET I ON A.PERMOHONAN_PAKET_ID = I.PERMOHONAN_PAKET_ID
						LEFT JOIN USER_LOGIN C ON I.USER_LOGIN_ID=C.USER_LOGIN_ID
						LEFT JOIN CONTRACTING_PROSES D ON B.CONTRACTINGPROSESID=D.CONTRACTINGPROSESID
						WHERE A.NAMA IS NOT NULL 
					) A WHERE 1=1
				 ";
		...
		$str .= $statement.' GROUP BY A.NAMA, A.TAHUN_PERMOHONAN, A.DEPARTMENT, A.CONTRACTINGREKANANID, A.CP_NAME, A.TAHUN_ANGGARAN ';
		// echo $str; die();
		$this->query = $str;
		return $this->selectLimit($str,$limit,$from);
  	}
  	...
  	function selectKontrakProsesSum($paramsArray=array(), $statement='')
	{
		$str = "SELECT SUM(A.NILAI_KONTRAK) AS ROWCOUNT
                FROM ( 
	                SELECT A.* FROM (
						SELECT A.PAKET_ID, A.NAMA, date_part('year'::text, I.rencana_pengadaan) AS TAHUN_PERMOHONAN, I.TAHUN_ANGGARAN, B.CONTRACTINGREKANANID, SUM(C.CR_NILAI_KONTRAK) NILAI_KONTRAK, SUM(C.CR_SPPBJ_NILAI) NILAI_SPPBJ
						FROM PAKET A
						JOIN CONTRACTING_REKANAN B ON A.PAKET_ID=B.PAKET_ID
						LEFT JOIN CONTRACTING_REKANAN_PROSES1 C ON B.CONTRACTINGREKANANID=C.CONTRACTINGREKANANID
						LEFT JOIN PERMOHONAN_PAKET I ON A.PERMOHONAN_PAKET_ID = I.PERMOHONAN_PAKET_ID
						WHERE A.NAMA IS NOT NULL 
						GROUP BY A.PAKET_ID, A.NAMA, date_part( 'year' :: TEXT, I.rencana_pengadaan ), B.CONTRACTINGREKANANID, I.TAHUN_ANGGARAN
					) A 
					WHERE 1=1
					";
		while(list($key,$val)=each($paramsArray))
    ...
    function selectKontrakSelesai($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = "SELECT A.*, 
				(SELECT SUM(Z.CR_NILAI_KONTRAK) FROM CONTRACTING_REKANAN_PROSES1 Z WHERE A.CONTRACTINGREKANANID=Z.CONTRACTINGREKANANID ) AS CR_NILAI_KONTRAK FROM (
						SELECT A.NAMA, date_part('year'::text, I.rencana_pengadaan) AS TAHUN_PERMOHONAN, I.TAHUN_ANGGARAN, B.CONTRACTINGREKANANID, C.DEPARTMENT
						FROM PAKET A
						JOIN CONTRACTING_REKANAN B ON A.PAKET_ID=B.PAKET_ID
						LEFT JOIN PERMOHONAN_PAKET I ON A.PERMOHONAN_PAKET_ID = I.PERMOHONAN_PAKET_ID
						LEFT JOIN USER_LOGIN C ON I.USER_LOGIN_ID=C.USER_LOGIN_ID
		...
		$str .= $statement.' GROUP BY A.NAMA, A.TAHUN_PERMOHONAN, A.DEPARTMENT, A.CONTRACTINGREKANANID, A.TAHUN_ANGGARAN';
		// echo $str; die();
		$this->query = $str;
		return $this->selectLimit($str,$limit,$from);
  	} 
