...
class paket_json extends CI_Controller {
			...
			<table class="table"> 
                ...
                ?>
                    <tr>
                        ...
                        <td width="90%"><a href="main/index/paket_lelang_detil/?reqId=<?=$paket->getField("PAKET_ID")?>"><?=strtoupper($paket->getField("NAMA"))?></a><br>
	...
	function paket_lelang()
	{
		...
		if(isset($reqPage)){
			...
			{
        $reqArrStatement = array("JENIS_PENGADAAN" => "LELANG"); 
			...
			elseif($this->USER_TYPE_ID == 8 || $this->USER_TYPE_ID == 9 || $this->USER_TYPE_ID == 10) // 8, 9:FRONTLINER, 10:HUKUM
			{
        $reqArrStatement = array("JENIS_PENGADAAN" => "LELANG"); 
			...
			{
        if((int)$this->USER_TYPE_ID == 3 || $this->USER_TYPE_ID == 7 || $this->USER_TYPE_ID == 10) // 3:PANITIA, 7:EKSEKUTIF, 10:HUKUM
          $reqArrStatement = array("JENIS_PENGADAAN" => "LELANG");
        else
          $reqArrStatement = array("JENIS_PENGADAAN" => "LELANG", "PUBLISH_PAKET" => "1");
                    ...
                    <div class="col-md-12" style="padding:0 !important">
                      <div class="card mb-1 <?= $border ?> border-darken-1">
                        <div class="card-content">
                          <div class="p-1">
                              <p class="mb-0">
                                <strong><a style="<?= $style ?>" href="main/index/paket_lelang_detil/?reqId=<?=$paket->getField("PAKET_ID")?>" ><?php // $nomor; ?> <?=strtoupper($paket->getField("NAMA"))?></a></strong>
                                
                                <?php
                                if ($paket->getField("REKANAN_ID_PEMENANG") !='') 
                                {
                                  if($this->USER_TYPE_ID == 3 || $this->USER_TYPE_ID == 7) // 3.Panitia, 7.Kepala Pengadaan
                                  {
                                ?>
                                <small class="text-muted float-right">
                                  <span class="badge badge-success" style="padding: 5px 10px">
                                    <a href="main/index/paket_penilaian/?reqId=<?=$paket->getField("PAKET_ID")?>"> <i class="fa fa-check"></i> Penilaian </a>
                                  </span>
                                </small>
                                <?php 
                                  }
                              ...
                              <p class="mb-0">
                                <small><i class="fa fa-calendar"></i> Tahun Anggaran</small> <?=getYear($paket->getField("TANGGAL_TAHAP"))?>
                              </p> 
                              <p class="mb-0">
                                <small><i class="fa fa-map-marker"></i> Lokasi Pekerjaan</small> <?=$paket->getField("LOKASI")?>
                              </p> 
                              <p class="mb-0 mt-1">
                                <? if($batal == 1) { ?>
                                 <span class="mt-1 mb-0 badge badge-danger">
                                  <i class="fa fa-refresh"></i> Paket Dibatalkan / Diulang 
                                 </span><br>
                                <? } 
                                ...
                                ?>
                                 ...
                                 if(($paket->getField("PAKET_METODE_LELANG_ID") == "1" ||  $paket->getField("PAKET_METODE_LELANG_ID") == "9"))
                                 {
                                 ...
                                 ?>
                                 <span class="badge badge-secondary">
                                   <span class="fa fa-check"></span> <a onClick="validasiPublish('<?=$paket->getField("PAKET_ID")?>')"> Validasi </a>
                                 </span>
                                 <?  
                                   }
                                  }
                                ...
                                ?>
                                 ...
                                 if($this->USER_TYPE_ID == 6 && $pendaftaran == 1)
                                 {
                                  ...
                                  if($mengikuti == 1)
                                  {
                                   ...
                                   {
                                  ?>
                                    <span class="badge badge-danger"><?=translate("Anda tidak memenuhi syarat pendaftaran, untuk melengkapi klik tombol daftar ulang.", "You are not eligible for registration, please click the button below to re-registration")?></span>
                                    <span class="badge badge-info">   
                                  ...
                                  } else
                                  {
                                   ...
                                   {  // karna 3 Kualifikasi Kecil / Non-Kecil
                                    $statement = " AND REKANAN_KUALIFIKASI_ID = '".$paket->getField("REKANAN_KUALIFIKASI_ID")."' AND STATUS_VALIDASI=1 AND A.REKANAN_ID='".$this->REKANAN_ID."' ";
                                    $cekKualifikasi = $bidang_usaha_rekanan->selectByParamsRekananCount(array(), -1, -1, $paket->getField("PAKET_ID"), $statement);
                                    if ($cekKualifikasi==1) { 
                                  ?>
                                   <span align="center">   
                                    <a href="main/index/registrasi_paket/?reqPaketId=<?=md5($this->REKANAN_ID.$paket->getField("PAKET_ID"))?>" class="btn btn-primary"><?=translate("Daftar", "Register")?></a>
                                   </span>
                                  <?
                                    } else {
                                      echo '<span class="badge badge-danger">Anda tidak memenuhi syarat pendaftaran</span>';
                                    }
                                   } else {
                                    ?>
                                     <span align="center">   
                                      <a href="main/index/registrasi_paket/?reqPaketId=<?=md5($this->REKANAN_ID.$paket->getField("PAKET_ID"))?>" class="btn btn-primary"><?php echo $pendaftaran; ?> <?=translate("Daftar", "Register")?></a>
                                     </span>
                                  <?php
                                   }
                                  }
  ...
  function paket_lelang2()
  {
    ...
    if(isset($reqPage)){
      ...
      if($this->USER_TYPE_ID == 6) // Rekanan
      {
        $reqArrStatement = array("JENIS_PENGADAAN" => "LELANG", "PAKET_METODE_LELANG_ID|| IN " => "(5,6)"); 
        $rowCount = $paket->getCountByParamsPaketRekanan($reqArrStatement, $this->REKANAN_ID, $statement);
        $paket->selectByParamsPaketRekanan($reqArrStatement, $dsplyRange, $dsplyStart, $this->REKANAN_ID, $statement);
      }
      elseif($this->USER_TYPE_ID == 8 || $this->USER_TYPE_ID == 9 || $this->USER_TYPE_ID == 10) // 8, 9:FRONTLINER, 10:HUKUM
      ...
      else
      {
        if((int)$this->USER_TYPE_ID == 3 || $this->USER_TYPE_ID == 7 || $this->USER_TYPE_ID == 10) // 3:PANITIA, 7:EKSEKUTIF, 10:HUKUM
          ...
          $reqArrStatement = array("JENIS_PENGADAAN" => "LELANG", "PUBLISH_PAKET" => "1", "PAKET_METODE_LELANG_ID|| IN " => "(5,6)");
      ...
      //initialize pagination class
      $pagConfig = array('baseURL'=>'paket_json/paket_lelang2', 'showRecord' => $reqShow, 'totalRows'=>$rowCount, 'currentPage'=>$dsplyStart, 'perPage'=>$dsplyRange, 'contentDiv'=>$reqContent, 'searchText' => $reqPencarian, 'arrSerialized' => $this->input->post("array_serialized"));
      $pagination =  new Pagination($pagConfig);
	...
	function set_publish_paket()
	{
		...
		{
			echo "Paket belum divalidasi semua panitia.";
			return;
		}
