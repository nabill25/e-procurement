...
class katalog_laporan_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns 			= array('LAPORANID','NAMA','ALASAN','JENISLAPORAN','NAMAPRODUK');
		$aColumnsAlias		= array('LAPORANID','NAMA','ALASAN','JENISLAPORAN','NAMAPRODUK');
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY LAPORANID desc" )
			{ 
				$sOrder = " ORDER BY A.LAPORANID ASC";
   ...
   //      } 
		$statement .= "AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(A.JENISLAPORAN) LIKE '".strtoupper($_GET['sSearch'])."%')";
		$allRecord = $katalog_laporan->getCountByParams(array(), $statement);
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else
			$allRecordFilter =  $katalog_laporan->getCountByParams(array(), $statement);

        if($this->USER_TYPE_ID == "6") { 
        	// $statement .= ' AND A.REKANAN_ID = '.$this->ID.'';
			$katalog_laporan->selectByParams(array(), $dsplyRange, $dsplyStart, $statement, $sOrder);
		} else {
			$katalog_laporan->selectByParams(array(), $dsplyRange, $dsplyStart, $statement, $sOrder);
		}
		...
		while($katalog_laporan->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i]=='NO')
					...
					$row[] = $katalog_laporan->getField(trim($aColumns[$i]));
			}
			$output['aaData'][] = $row;
		}
	...
	function delete()
	{
		...
		$katalog_laporan->setField('LAPORANID', $reqId);
		if($katalog_laporan->delete())
