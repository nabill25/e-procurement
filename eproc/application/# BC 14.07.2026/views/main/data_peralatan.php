        ...
        <div class="row row-treegrid">
        	<div class="col-md-12 col-treegrid">
            	<div class="area-konten-atas">
                	<div class="judul-halaman">Peralatan
                    	<div class="info">
							<i class="fa fa-warning" aria-hidden="true"></i> Double-click untuk memilih peralatan.  
                        </div>                    
                    </div>
                    
                </div>
                <div id="idpencarian" class="area-pencarian">
                    <label>Ketik nama peralatan : <input class="" placeholder="" name="reqPencarian" aria-controls="example" type="search"></label>
                </div>
				<div id="tableContainer" class="tableContainer tableContainer-treegrid">
                	<table id="treeSatker" class="easyui-treegrid" style="width:700px;height:300px"
                            data-options="
                                url: 'rekanan_peralatan_json/get_data_peralatan',
                                method: 'get',
                                idField: 'id',
                                treeField: 'text'
                            ">
                        <thead>
                            <tr>
                                <th data-options="field:'JENIS'" >Nama</th>
                                <th data-options="field:'KAPASITAS'" >Kapasitas</th>
                                <th data-options="field:'MERK'" >Merk</th>
    ...
    $(document).ready( function () {
		$('input[name=reqPencarian]').keypress(function( event ) {
			if ( event.which == 13 ) {
				...
				var urlApp = 'rekanan_peralatan_json/get_data_peralatan/?reqSearch='+ value;
				$('#treeSatker').treegrid(
				{
					url: urlApp
				});	
        ...
        $('#treeSatker').treegrid({
              onDblClickRow: function(param){
				 ...
				 if($reqMode == "KUALIFIKASI")
				 	echo "top.addKualifikasiPeralatan(param.id, param.JENIS); ";
				 else
				 	echo "top.tambahPeralatan(param.id, param.JENIS);";
				 ?>				  
