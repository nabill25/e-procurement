...
<html lang="en">
  <head>
    ...
    $(function(){
      $('#ffAddMaterial').form({
        url:'contracting_json/editMaterial',
        onSubmit:function(){
          ...
          <div class="p-1" >
            <form id="ffAddMaterial" class="easyui-form form-horizontal" method="post" novalidate enctype="multipart/form-data" style="padding:0 50px">
              <div class="table-responsive">
                <table class="table table-bordered table mb-0">
                  ...
                  <tbody id="tbodyDeliverable">  
                    ...
                    $datamaterial->selectByParams(array("MATERIALID"=>$reqId));
                    $no=21;
                    ...
                    <tr>
                      <td>
                       ...
                       <td>
                        <input type="text" class="form-control easyui-validatebox" required name="keterangan[]" id="<?=$no?>" value="<?= $datamaterial->getField('KETERANGAN') ?>">
                       </td> 
                       <td>
                        <input type="text" class="form-control easyui-validatebox" required name="hargasatuan[]" id="reqhargasatuan<?=$no?>" OnFocus="FormatAngka('reqhargasatuan<?=$no?>')" OnFocus="FormatAngka('reqhargasatuan<?=$no?>')" OnKeyUp="FormatUang('reqhargasatuan<?=$no?>')" OnBlur="FormatUang('reqhargasatuan<?=$no?>')" value="<?= $datamaterial->getField('HARGA_SATUAN') ?>">
                       </td> 
                    </tr>
                    <?php 
                    $no++;
              ...
              <div class="form-actions">
                <input type="hidden" name="reqMaterialid" id="reqMaterialid" value="<?=$reqId?>"/>
