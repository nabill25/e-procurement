...
$reqBastPekerjaanTanggal = $proses5->getField('CR_BAST_PEKERJAAN_TANGGAL') ?: '-';   
$reqBastPekerjaanNamaPenyedia = $proses5->getField('CR_BAST_PEKERJAAN_NAMA_PENYEDIA') ?: '-';   
$reqBastPekerjaanJabatanPenyedia = $proses5->getField('CR_BAST_PEKERJAAN_JABATAN_PENYEDIA') ?: '-';   
$reqBastPekerjaanNamaPenerima = $proses5->getField('CR_BAST_PEKERJAAN_NAMA_PENERIMA') ?: '-';   
$reqBastPekerjaanJabatanPenerima = $proses5->getField('CR_BAST_PEKERJAAN_JABATAN_PENERIMA') ?: '-';   
      ...
      <div class="card-content collapse show border-info border-darken-2">
        <div class="card-body">
            ...
            <div class="card-content">
              <div class="p-1">
                ...
                <small style="font-size:0.7em">
                  <a onclick="openAdd('main/loadUrlKontrak/kontrak/contracting_file_add?reqAidi=<?= $reqId ?>&reqProses=5&reqJenis=BAST Pekerjaan')" class="badge badge-primary pull-right mr-1 text-white"> <i class="fa fa-plus-circle"></i> Tambah Dokumen BAST </a> 
                </small>
                <?php 
                } ?>
                </h5>
                <div class="table-responsive">
                  <table id="dokumenFileIdTable" class="border-double table mb-0 table-bordered mb-2" style="width: 100%">
                    <?= $this->libkontrak->getTableFile($reqId," AND A.FILE_JENIS = 'BAST Pekerjaan' ") ?>
                  </table>
                </div>
              </div>
            </div>
          ...
          <div class="form-actions">
            ...
            <a onclick="openAdd('main/loadUrlKontrak/kontrak/contracting_serah_terima_hasil_edit?reqId=<?= $reqId ?>')" class="<?= CLASS_BTN_PRIMARY ?> mb-1"> <?= BTN_UBAH ?> Data BAST Pekerjaan </a> 
            <?php 
            }?>
              ...
              <tbody> 
                <tr> 
                  <td width="25%" colspan="2">
                    <small>Nomor BAST</small> <br> <?= $reqBastPekerjaanNomor ?>
                  </td> 
                  <td width="25%" colspan="2">
                    <small>Tanggal BAST</small> <br> <?= getFormattedDate($reqBastPekerjaanTanggal) ?>
                  </td> 
                </tr> 
                <tr>
                  <td width="25%" colspan="2">
                    <small>Nama Penyedia</small> <br> <?= $reqBastPekerjaanNamaPenyedia ?>
                  </td> 
                  <td width="25%" colspan="2">
                    <small>Jabatan Penyedia</small> <br> <?= $reqBastPekerjaanJabatanPenyedia ?>
                  </td> 
                </tr> 
                <tr>
                  <td width="25%" colspan="2">
                    <small>Nama Penerima</small> <br> <?= $reqBastPekerjaanNamaPenerima ?>
                  </td> 
                  <td width="25%" colspan="2">
                    <small>Jabatan Penerima</small> <br> <?= $reqBastPekerjaanJabatanPenerima ?>
                  </td> 
                </tr>  
                <tr>
                  <td width="20%" colspan="4">
                    ...
                    <?php  
                    if(str_replace(' ','',$reqBastPekerjaanStatus) == '1') {
