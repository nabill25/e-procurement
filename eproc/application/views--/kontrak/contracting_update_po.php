...
$this->load->model(array("Contractingrekanan"));
...
<html lang="en">
  <head>
    ...
    $(function(){
      $('#ff').form({
        url:'contracting_json/update_po',
        onSubmit:function(){
    ...
    <div class="card-content">
      <div class="p-1">
        <div class="alert bg-primary alert-icon-left alert-arrow-left alert-dismissible mb-2" role="alert">
          <span class="alert-icon"><i class="fa fa-th"></i></span>  <strong>Update No. PO</strong>
        </div>
        <div class="p-1">
         <form id="ff" method="post" class="form-horizontal" role="form" style="padding: 0 12px;">
            <table class="table table-bordered table-hover p-1">
                <tbody>
                  <tr>
                    <td width="27%">Nomor PO</td>
                    <td width="75%"> 
                      <input type="text" class="form-control easyui-validatebox" required name="reqNoPO" id="reqNoPO" value="<?= $contracting->getField('CR_PO') ?>" required>
                    </td>
                  </tr>
                </tbody>
             </table> 
            ...
            <div class="form-actions">
              <input type="hidden" name="reqId" value="<?=$reqId?>"> <!-- contractingrekananid -->
              <input type="hidden" name="reqPaketId" value="<?=$contracting->getField('PAKET_ID')?>"> <!-- paketid -->
              <input type="hidden" name="reqMode" value="<?=isset($reqMode)?$reqMode:''?>">
              <a href="#" onClick="top.closePopup()" class="<?= CLASS_BTN_DANGER ?> pull-right"> <i class="fa fa-close"></i> Tutup</a>
          ...
          echo $libkontrak->getInfoKontrak($reqId);
          ?>
