...
$(document).ready(function() {
	$(function(){
		$('#ff').form({
			url:'paket_rekanan_json/pengumuman_pemenang',
			onSubmit:function(){
				return $(this).form('validate');
			},
			success:function(data){
				//alert(data);
				document.location.href = 'main/index/paket_lelang_tambah_pengumuman_pemenang/?reqId=<?=$reqId?>';
			}
		});
	});
});
  ...
  <div class="col-md-12 col-sm-12">
    <div class="card">
      <div class="card-header card-head-inverse bg-primary">
        <h4 class="card-title text-white">Pengumuman Pemenang <?= $paketInfo->metode_lelang_nama ?></h4>
      ...
      <div class="card-content collapse show border-info border-darken-2">
        <div class="card-body area-datatable">
          <div class="row">
            <div class="form-group col-md-12 mb-2">
              <label >Lampiran</label>
                  ...
                  <input type="file" name="reqLinkFile" id="reqLinkFilePDF" size="30" <? if($reqLinkFileTemp == "") { ?> required <? } ?> maxlength="1" class="easyui-validatebox"  validType="fileType['pdf']" />
                  ...
                  <? if ($paket_dokumen->getField("PATH_FILE")=='')
                     ...
                     {
                  ?>
                  <br>temp : <?=$paket_dokumen->getField("PATH_FILE")?> <br>
              		  <iframe src="<?=$FILE_DIR.str_replace("'", "''", $paket_dokumen->getField("PATH_FILE"))?>" style="width:100%; height:600px;"></iframe>
                	<?
						         }
					         ?>
          ...
          <div class="form-actions">
            ...
            <button type="submit" id="submitUpload" class="btn btn-primary"><i class="fa fa-upload"></i> Upload</button>
          </div> 
        </div>
      </div>
      </form>
