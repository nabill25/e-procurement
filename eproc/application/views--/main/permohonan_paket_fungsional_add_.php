...
if($reqId == '')
{
  ...
  $reqMetodePengadaan = $permohonan_paket->getField("PENGADAANLANGSUNG");
  if ($reqNilai == '') {
  ...
  $(function(){
    $('#ff').form({
      ...
      onSubmit:function(){
        ...
        // if(v) $.messager.progress();  // show the message box
        if(v) showLoad();  // show the message box
        return v;
      },
      success:function(data){
        // $.messager.alert('Info', data, 'info'); false;
        var hasil=data.split('-');
        hideLoad();
        // $.messager.progress('close');  // close the message box
        // alert(hasil[1]);
        // return false;
        // document.location.href = 'main/index/permohonan_paket_fungsional/?reqId=<?=$reqId?>';   // Old
  ...
  <div class="col-md-12 col-sm-12">
    <div class="card">
      <div class="card-header card-head-inverse bg-primary">
        <h4 class="card-title text-white">Permohonan Paket
        <?php
      ...
      <div class="card-content collapse show border-info border-darken-2">
          ...
          <div class="row">
            <div class="form-group col-md-12 mb-12">
              <label>Nilai HPS <!-- <span id="setNilaiHPS">diatas Rp.1.000.000.000,-</span></label> -->
              <input type="text" name="reqNilai" id="reqNilai" onchange="return cekReqNilai()" class="form-control span3 easyui-validatebox" required value="<?=numberToIna($reqNilai)?>" OnFocus="FormatAngka('reqNilai')" OnKeyUp="FormatUang('reqNilai')" OnBlur="FormatUang('reqNilai')" />
            </div>
          </div>
                ...
                <div class="table-responsive">
                  <table class="border-double table mb-0">
                    ...
                    <tbody id="tbodyPermohonanCOA">
                      ...
                      $noRand = 2021;
                      $permohonan_paket_coa->selectByParamsCoa(array("A.PERMOHONAN_PAKET_ID" => coalesce($reqId, 0)));
                      ...
                      <tr>
                        <td>
                          <input type="text" name="reqNomorCOA[]" id="reqNomorCOA<?=$noRand?>" class="form-control span3 easyui-validatebox" value="<?= $permohonan_paket_coa->getField("NOMOR") ?>" />
                       </td>
                       <td>
                          <input type="text" name="reqKeteranganCOA[]" id="reqKeteranganCOA<?=$noRand?>" class="form-control span3 easyui-validatebox" value="<?= $permohonan_paket_coa->getField("KETERANGAN") ?>"/>
                       </td>
                       <td>
                          <input type="text" name="reqBudgetAwal[]" id="reqBudgetAwal<?=$noRand?>" class="form-control span3 easyui-validatebox" OnFocus="FormatAngka('reqBudgetAwal<?=$noRand?>')" OnKeyUp="FormatUang('reqBudgetAwal<?=$noRand?>')" OnBlur="FormatUang('reqBudgetAwal<?=$noRand?>')" onchange="calculate('<?=$noRand?>');"  value="<?= $permohonan_paket_coa->getField("BUDGET_AWAL") ?>"/>
                       </td>
                       <td>
                          <input type="text" name="reqBudgetTerpakai[]" id="reqBudgetTerpakai<?=$noRand?>" class="form-control span3 easyui-validatebox" OnFocus="FormatAngka('reqBudgetTerpakai<?=$noRand?>')" OnKeyUp="FormatUang('reqBudgetTerpakai<?=$noRand?>')" OnBlur="FormatUang('reqBudgetTerpakai<?=$noRand?>')" onchange="calculate('<?=$noRand?>');"  value="<?= $permohonan_paket_coa->getField("BUDGET_TERPAKAI") ?>"/>
                       </td>
                       <td>
                          <input type="text" name="reqBudgetAkhir[]" id="reqBudgetAkhir<?=$noRand?>" class="form-control span3 easyui-validatebox" OnKeyUp="FormatUang('reqBudgetAkhir<?=$noRand?>')" OnBlur="FormatUang('reqBudgetAkhir<?=$noRand?>')" onchange="calculate('<?=$noRand?>');"   value="<?= $permohonan_paket_coa->getField("BUDGET_AKHIR") ?>"/>
                       </td>
                       <td>
                  ...
                  <a onclick="createRowCOA()" class="btn btn-primary m-1 text-white">
            ...
            <div class="card-content">
              <div class="p-1">
                ...
                <div class="table-responsive">
                  ...
                  <a onclick="createRowNotaDinas()" class="btn btn-primary m-1 text-white">
          ...
          <div class="form-actions">
            ...
            <a href="main/index/permohonan_paket_fungsional" class="btn btn-danger mr-1 text-white"> <i class="fa fa-arrow-left"></i> Kembali </a>
            <button type="submit" class="btn btn-primary "> <i class="fa fa-check-square-o"></i> Simpan</button>
            <?php
            if ($reqId != '' && $reqMetodePengadaan != '2') { ?>
              <a href="main/index/paket_lelang_tambah_rincian_pekerjaan_permohonan/?reqPer=<?= $reqId ?>" class="btn btn-primary pull-right mr-1 text-white"> Lanjut <i class="fa fa-arrow-right"></i></a>
            <?php
            } ?>
          </div>
