...
$dataInbox    = new Inbox();
$arrStatement = array("A.INBOXID" => $id, "A.PARENT" => 0, "A.INBOXCATEGORYID" => '2', "A.PENERIMA" => $this->REKANAN_ID);
$dataInbox->selectByPenerima($arrStatement, -1, -1);
$dataInbox->firstRow();

if ($dataInbox->getField("INBOX_SUBJECT") == '')
	redirect(base_url());

$inboxid = $dataInbox->getField("INBOXID");
$inbox_subject = 'Replay - '.$dataInbox->getField("INBOX_SUBJECT");
...
function backP() {
	window.location.href = "main/index/inbox_survei_penyedia";
}

$(document).ready(function() {
	$(function(){
		$('#ff').form({
			url:'inbox_rfi_json/replay',
			onSubmit:function(){
	      ...
	      success:function(data){
	        // $.messager.alert('Info', data, 'info');
	          ...
	          alertSuccess2(data);
	          setTimeout(function () {
	            window.location.href = '<?= base_url('main/index/inbox_survei_penyedia') ?>';
	          }, 1000);
	        hideLoad();
	      }
	    });
          					...
          					<form id="ff" class="easyui-form form-horizontal" method="post" novalidate enctype="multipart/form-data">
								...
								<div class="row">
								    <div class="form-group col-md-12 mb-1">
								      ...
								      <input type="text" name="reqFrom" maxlength="255" accesskey="n" title="Pengirim harus diisi" class="form-control easyui-validatebox span7" value="<?=isset($penerima_str)?$penerima_str:''?>" id="reqFrom" required />
								    </div>
								</div>
								<div class="row">
								  <div class="form-group col-md-12 mb-1">
								...
								<div class="row">
									<div class="form-group col-md-12">
						                <label style="width: 100%">Upload lampiran <small>(Format file .pdf .zip & Maksimal ukuran file 2MB) </small></label>
						                 <input type="file" name="reqLinkFile" id="reqLinkFilePDF" size="30" maxlength="1" class="form-control easyui-validatebox" required validType="fileType['pdf','zip']" />
						            </div>
						        </div>
