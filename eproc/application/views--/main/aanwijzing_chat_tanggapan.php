...
$(document).ready(function() {
	...
	$(function(){
		$('#ff').form({
			url:'aanwijzing_chat_json/dokumen_aanwijzing_tanggapan',
			onSubmit:function(){
				return $(this).form('validate');
			},
			success:function(data){
				// alert(data);return false;
				document.location.href = 'main/index/aanwijzing_chat_tanggapan/?reqId=<?=$reqId?>&reqParent=<?=$reqParent?>';
			}
		});
...
function edit(paketid,paketaanwijzingid) {
  openAdd("main/loadUrl/main/aanwijzing_chat_tanggapan_edit/?reqId="+paketid+"&paketaanwijzingid="+paketaanwijzingid);
}

function reloadMonitoringHere()
{
  setTimeout( function() 
  {
    location.reload();
  ...
  <div class="col-md-12 col-sm-12">
    <div class="card">
      <div class="card-header card-head-inverse bg-primary">
        <h4 class="card-title text-white">Jawaban Aanwijzing 
        </h4>
              ...
              <tbody>
                ...
                <?php
                  ...
                  while($paket_aanwijzing->nextRow())
                  {
                    ...
                    $paket_aanwijzing_parent->selectByParams(array("PAKET_ID" => $reqId, "PARENT_ID" => $reqParent ));
                ?>
                <tr >
                    <td>
                      <i class="fa fa-user"></i> <?=$paket_aanwijzing->getField("KODE_CUT")?><br>
                        <!-- <small><?php // echo $paket_aanwijzing->getField("NAMA_PENYEDIA") ?></small> -->
                    </td> 
                    <td>
                ...
                ?>
                  <tr >
                    ...
                    <a onclick="edit(<?= $reqId ?>,<?php echo $paket_aanwijzing_parent->getField("PAKET_AANWIJZING_ID")?>)" class="btn-aksi btn">
                      <?= ICON_EDIT ?>
                    </a>
                    <a onClick="deleteData('aanwijzing_chat_json/delete/', '<?php echo $paket_aanwijzing_parent->getField("PAKET_AANWIJZING_ID")?>')" class="btn-aksi">
                      <?= ICON_DELETE ?>
                    </a>
                    </td>
                </tr>
