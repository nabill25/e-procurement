...
while($paket_rekanan->nextRow())
{
  ...
  $arrRekananHadirPembukaan[] = $paket_rekanan->getField("HADIR_PEMBUKAAN_PENAWARAN2");
...
$arrPembukaanAuction            = PEMBUKAAN_AUCTION_SAMPUL2;
...
{
  $.messager.confirm("Konfirmasi","Publish berita acara pembukaan penawaran file 2 ?",function(r){
    if (r){
      $.get( "paket_json/set_publish_pembukaan_sampul2/?reqId=<?=$reqId?>", function( data ) {
          if(data == "1")
          {
            $("#btnPublish").css("display", "none");
            $.messager.alert('Info', "Publish pembukaan penawaran file 2 berhasil.", 'info');
          }
          else
            $.messager.alert('Info', data, 'info');
      });
  ...
  <div class="col-md-12 col-sm-12">
    <div class="card">
      <div class="card-header card-head-inverse bg-primary">
        <h4 class="card-title text-white">Pembukaan Penawaran Pengadaan File 2</h4>
                ...
                <tr class="judul-kolom">
                  ...
                  ?>
                    <th class="alert" style="font-size: 15px; font-weight: bold; text-align: center">
                      <?=$arrRekanan[$i]?>
                      <br>
                      <a href="<?= base_url('evaluasi_download_json/aanwijzing_publish_json?reqId='.$reqId.'&rekanan='.$arrRekananId[$i].'&file=2&tahap=all') ?>" class="btn btn-sm round btn-min-width box-shadow-1 btn-success text-white" target="_blank"><span class="fa fa-download"></span> Download semua dokumen</a>       
                    </th>
                  <?php 
                  }
                  ?>
                    ...
                    <?php 
                      ...
                      <th align="center" style="text-align: center">
                        ...
                        if ($arrPasswordDokumen[$i]) {
                          echo '<span class="fa fa-copy" onclick="jscopy(\''.$arrRekanan[$i].'\')" style="cursor:pointer"> &nbsp;&nbsp; Copy Password</span><span id="'.$arrRekanan[$i].'" style="display:none">'.$arrPasswordDokumen[$i].'****</span>';
                        } else {
                  ...
                  <tr class="gelap">
                    <td class="alert-info" colspan="<?=2+(count($arrRekanan))?>"><strong> PENAWARAN HARGA</strong></td>
                  </tr>
                  ...
                  <tr class="terang">
                    ...
                    <td style="width: 28%"><div class="uraian"> Nilai Penawaran </div></td>
                    <?php 
                    $adaDiatasOE = 0;
                    for($j=0;$j<count($arrRekanan);$j++)
                    {
                    ?>
                      <td align="center">
                        ...
                        <input style="width:150px !important" name="reqDataPenawaranHarga[]" class="form-control easyui-validatebox span2" type="text" id="reqDataPenawaranHarga<?=$j?>" value="<?= number_format($arrPaketRekananNilai[$j],2, ",",".");?>" readonly="readonly"/>
                      </td>
                      <?php 
              ...
              <form id="ff" method="post" novalidate enctype="multipart/form-data">
                ...
                <div class="form-actions">
                  ...
                  <a href="main/index/paket_detil/?eid=<?=$reqId?>&key=<?=$reqUUID?>" class="<?= CLASS_BTN_DANGER ?> mr-1"> <?= BTN_KEMBALI ?></a>
                    ...
                    if ($info == 1)
                    { ?>
                    <a href="main/loadUrl/report/dokumen_pembukaan_penawaran_sampul2_ba_pdf/?reqId=<?=$reqId?>" target="_blank" class="<?= CLASS_BTN_INFO ?> mr-1"><?= BTN_PRINT ?> Hasil Pembukaan</a>
                    <?php
                    } ?>
                    ...
                    else
                    {
                      ...
                      {
                        if($paket_pembukaan_validasi->getField("KODE") == "" && $info == 1)
                        {
                      ?>
                      ...
                      if($paket_pembukaan_validasi->getField("JENIS") == "PEMBUAT")
                      {
                        ...
                        {
                        ?>
                          <a onClick="publishPembukaanSampul2();" id="btnPublish" class="<?= CLASS_BTN_SUCCESS ?>"><?= BTN_PUBLISH ?></a>
                        <?php 
                        }
                      }
                    }
