...
class paket_json extends CI_Controller {
  ...
  function penawaranbypass()
  {
    $this->load->model(array("PaketRekanan","PaketNegoisasi"));
    ...
    $paket_rekanan->setField("PAKET_PENAWARAN_ID",$reqPaketPenawaranId);
    $paket_rekanan->setField("NILAI_PENAWARAN",CommaToDot(dotToNo($reqPenawaran)));
    $paket_rekanan->setField("NILAI_NEGOSIASI",CommaToDot(dotToNo($reqJumlahNegosiasi)));
    $paket_rekanan->setField("CREATED_BY", $this->USER_LOGIN_ID);

    if ($paket_rekanan->updateNegoBypass()) { 
        echo "Data berhasil di simpan";
    } else {
      echo "Data gagal di simpan, silahkan coba kembali.";
    }

  }

  function negosiasibypass()
  {
    $this->load->model(array("PaketRekanan","PaketNegoisasi"));
    ...
    $paket_rekanan->setField("REKANAN_ID", $reqRekananId);
     ...
     if ($paket_negosiasi->countRow() > 0) {
       $paket_rekanan->update2NegoBypass();
        echo "Data berhasil di simpan";
     } else {
       $paket_rekanan->insertNegoBypass();
        echo "Data berhasil di simpan";
     }
