...
class paket_rekanan_json extends CI_Controller {
	...
	function undang_pemilihan_bypass()
	{
		...
		if($submitSimpan == "Simpan")
		{
			...
			for($i=0; $i<count($reqRekananId);$i++)
			{
				...
				if($check == 0)
				{
					...
					$paket_rekanan_check->setField("LULUS_PENDAFTARAN", 1);
					$paket_rekanan_check->setField("LULUS_KUALIFIKASI", 1);
					$paket_rekanan_check->setField("LULUS_PENAWARAN", 1);
					$paket_rekanan_check->setField("LULUS_ADMINISTRASI", 1);
					$paket_rekanan_check->setField("LULUS_PENAWARAN_KETERANGAN", "LULUS");
					$paket_rekanan_check->setField("KIRIM_PENAWARAN_KODE", "BYPASS");
					$paket_rekanan_check->setField("PAKET_PENAWARAN_ID", $reqPaketPenawaranId);
					$paket_rekanan_check->setField("LAST_CREATE_USER", $reqRekananId[$i]);
					$paket_rekanan_check->setField("KODE_REKANAN", date("ymd"));
					$paket_rekanan_check->insertUndang2Bypass();
				}
				unset($paket_rekanan_check);
			}
			// echo $data_tdk_delete.'-';
