...
class permohonan_paket_usulan_json extends CI_Controller {
	...
	function permohonan_usulan_add()
	{
		...
		$usulanId = $this->input->post("usulanId");
		$reqPermohonanPaketId = $this->input->post("reqPermohonanPaketId");
		$reqMode = $this->input->post("reqMode");
		$reqTahunAnggaran = $this->input->post("reqTahunAnggaran");
		$reqMataAnggaran = $this->input->post("reqMataAnggaran") ?: '';
		$reqKegiatan = $this->input->post("reqKegiatan") ?: '';
		$reqSumberDana = $this->input->post("reqSumberDana") ?: '';
		$reqBudgetRemaining = $this->input->post("reqBudgetRemaining") ?: 0;
		$reqDepartment = $this->input->post("reqDepartment");
		$reqDepartmentCode = $this->input->post("reqDepartmentCode") ?: '';
		$reqKodeMataAnggaran = $this->input->post("reqKodeMataAnggaran") ?: '';
		$reqKodeKegiatan = $this->input->post("reqKodeKegiatan") ?: '';
		$reqTotalBudget = $this->input->post("reqTotalBudget") ?: 0;
		$reqTipeTransaksi = $this->input->post("reqTipeTransaksi");
		$reqNamaKebutuhan = $this->input->post("reqNamaKebutuhan");
		$reqIdentifikasiResiko = $this->input->post("reqIdentifikasiResiko");
		$reqIdentifikasiResikoKeterangan = str_replace("'","''",$_POST["reqIdentifikasiResikoKeterangan"]);
		$reqAnalisaKategori = $this->input->post("reqAnalisaKategori");
		$reqAnalisaJenisBelanja = $this->input->post("reqAnalisaJenisBelanja");
		$reqNote = str_replace("'","''",$_POST["reqNote"]);
    	$reqSumberDanaKeterangan = str_replace("'","''",$_POST["reqSumberDanaKeterangan"]);

		// $reqKomoditas = $this->input->post("reqKomoditas");
		$reqAnalisaKebutuhan = $this->input->post("reqAnalisaKebutuhan");
		// $reqAnalisaPasar = $this->input->post("reqAnalisaPasar");
		$reqAnggaran = $this->input->post("reqAnggaran");
		$reqNamaPaket = $this->input->post("reqNamaPaket");
		$reqPerkiraanBiayaHarga = $this->input->post("reqPerkiraanBiayaHarga");
		$reqJenisBarangJasa = $this->input->post("reqJenisBarangJasa");

		// Gabung
		$reqWaktuPenggunaBarangjasa = '01-'.$this->input->post("reqWaktuPenggunaBarangjasaMonth").'-'.$this->input->post("reqWaktuPenggunaBarangjasaYear");
		$reqRencanaPengadaan = '01-'.$this->input->post("reqRencanaPengadaanMonth").'-'.$this->input->post("reqRencanaPengadaanYear");
		$reqCaraPengadaan = $this->input->post("reqCaraPengadaan");
		...
		if($reqMode == 'insert')
		{
			if($permohonan_paket_analisa->insertAnalisa())
			{
				...
				$permohonan_paket_anggaran_insert->setField("BUDGET_REMAINING", $reqBudgetRemaining);
			...
			// 		$permohonan_paket_coa->setField("PERMOHONAN_PAKET_ID", $permohonanId);
		...
		else
		{
			...
			if($permohonan_paket_analisa->updateAnalisa())
			{
				...
				$permohonan_paket_anggaran_select->selectByParamsAnggaran(array("A.PERMOHONAN_PAKET_ID" => $reqPermohonanPaketId));
				...
				if ($permohonan_paket_anggaran_select->countRow() > 0) { // update
					...
					$permohonan_paket_anggaran_insert->setField("TIPE_TRANSAKSI", $reqTipeTransaksi);
					$permohonan_paket_anggaran_insert->setField('CREATED_BY', $this->USER_LOGIN_ID);
					$permohonan_paket_anggaran_insert->updatePermohonanAnggaran();
				} else { // Insert
					$reqPermohonanPaketId = $this->input->post("reqPermohonanPaketId");
					$permohonan_paket_anggaran_insert = new PermohonanPaket();
					$permohonan_paket_anggaran_insert->setField("PERMOHONAN_PAKET_ID", $reqPermohonanPaketId);
			...
			// 	$permohonan_paket_coa_delete->setField("PERMOHONAN_PAKET_ID", $reqPermohonanPaketId);
			...
			// 			$permohonan_paket_coa->setField("BUDGET_TERPAKAI", dotToNo($reqBudgetTerpakai[$i]));
			// 			$permohonan_paket_coa->setField("BUDGET_AKHIR", dotToNo($reqBudgetAkhir[$i]));
			// 			$permohonan_paket_coa->setField("PERMOHONAN_PAKET_ID", $reqPermohonanPaketId);
			// 			$permohonan_paket_coa->setField('CREATED_BY', $this->USER_LOGIN_ID);
			// 			$permohonan_paket_coa->insertcoa();
			// 		}
			// 		unset($permohonan_paket_coa);
	...
	function setKodeRUP($usulanId,$userloginid)
	{
		...
		$kode = substr(date('Y'),2,4).'.RP.'.$user_login->getField("KODE").'.'.generateZero($usulanId, 3, 0);
	...
	function getMataAnggaran()
	{
        ...
        $libapi = new libapi();
        // echo $reqUrl.'-'.$reqDate.'-'.$reqDepartment;
        $a = $libapi->getAnggaran($reqUrl,$reqDate,$reqDepartment);
        // echo "<pre>"; print_r($a->results->data);
