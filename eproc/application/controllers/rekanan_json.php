...
class rekanan_json extends CI_Controller {
	...
	function daftar_rekanan_json()
	{
		...
		$aColumns 			= array('REKANAN_ID', 'KODE', 'NAMA','KOTA','TANGGAL_DAFTAR','UPDATED_DATE','USER_VALIDASI', 'USER_STATUS', 'USER_AKTIF','USER_LOGIN_ID');
		$aColumnsAlias		= array('REKANAN_ID', 'KODE', 'NAMA','KOTA','TANGGAL_DAFTAR','UPDATED_DATE','USER_VALIDASI', 'USER_STATUS', 'USER_AKTIF','USER_LOGIN_ID');
		...
		while($rekanan->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				...
				} elseif($aColumns[$i]=='UPDATED_DATE') {
					$mulai   = new DateTime($rekanan->getField(trim($aColumns[$i])));
					$selesai = $sekarang = new DateTime();
					$selisih = $mulai->diff($selesai);
					$row[] = ($selisih->days." Hari ".$selisih->h." Jam");
	...
	function rekanan_konfirmasi()
	{
		...
		if($user_login->updateStatus())
		{
			...
			$rekanan->setField("REKANAN_ID", $this->ID);
			$rekanan->setField("UPDATED_DATE", date('Y-m-d H:i:s'));
			$rekanan->updateByFieldAkmal();
			// $Ccs = array($_SESSION["ses_CabangEmail"]);
			// $cbg = str_replace(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR,'',dirname(__FILE__));
			// $mail = new KMail($cbg);
			// $mail->Subject  =  'Registrasi - '.SYSTEM_NAME.' '.SYSTEM_NAME_PT;
