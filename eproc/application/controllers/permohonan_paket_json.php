...
class permohonan_paket_json extends CI_Controller {
	...
	function permohonan_lelang_tunjuk_pic_json()
	{
		...
		$statement .= " AND (A.PENGADAAN_BYPASS = '' OR A.PENGADAAN_BYPASS is null OR A.PENGADAAN_BYPASS = '0')"; // Update 11-07-2026
	...
	function permohonan_lelang_bypass_json()
	{
		$this->load->library("kauth");  $userLogin = new kauth();
		$this->load->model("PermohonanPaket");
	 ...
	 //    {
		...
		$statement .= " AND A.PENGADAAN_BYPASS = '1' AND (APPROVE_PPK != '1' OR APPROVE_PPK IS NULL)"; // Update 11-07-2026
		...
		$permohonan_paket->selectByParamsBypass(array(), $dsplyRange, $dsplyStart, $statement.$searchJson, $sOrder);
		//echo "dsds".$permohonan_paket->query;exit;
		/* Output */
		$output = array(
			"sEcho" => intval($_GET['sEcho']),
