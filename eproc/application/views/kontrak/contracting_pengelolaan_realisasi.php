...
function publishFile(delele_link, id, stat)
  {
    if (stat == '1') {
      var messa = 'Publish Dokumen ke Penyedia ?';
    } else {
      var messa = 'Un-Publish Dokumen ke Penyedia ?';
    }
          ...
          <div class="form-actions table-responsive">
            <table id="deliveriable" class="border-double table mb-0 table-bordered mb-2" style="width: 100%">
              <tr class="backcolornew">
                <?php
                if ($this->LEGAL != '1' && ($this->LEVEL_KONTRAK == '2' && $reqPICPengendali == $this->USER_LOGIN_ID)) { // update status oleh peng. kontrak bukan legal ?>
                ...
                <th>Realisasi</th>
                <th>Keterangan</th>
                <th width="80px">Tanggal Realisasi</th>
              ...
              if ($datadelivery->countRow() > 0) {
                ...
                <tr>
                  <?php
                  if ($this->LEGAL != '1' && $this->USER_TYPE_ID != '20' && ($this->LEVEL_KONTRAK == '2' && $reqPICPengendali == $this->USER_LOGIN_ID) && $datadelivery->getField('FILE_BAPP') != '') { // update status oleh peng. kontrak bukan legal, bukan pemeriksa kontrak ?>
                  <td>
                    <a style="color: #fff;" onclick="openAddFrame('main/loadUrlKontrak/kontrak/contracting_add_deliverable_bapp_edit?reqAidi=<?= $datadelivery->getField('DELIVERABLEID') ?>&reqConRekId=<?= $reqId ?>')" class="badge badge-danger"><span class="fa fa-edit"></span> Update BAPP</a><br>
                    <a style="color: #fff;" onclick="openAdd('main/loadUrlKontrak/kontrak/contracting_add_deliverable_edit?reqAidi=<?= $datadelivery->getField('DELIVERABLEID') ?>&reqConRekId=<?= $reqId ?>')" class="badge badge-danger"><span class="fa fa-edit"></span> Update Realisasi</a>
                  </td>
                  <?php
                  } else { 
                    if ($this->LEVEL_KONTRAK == '2' && $reqPICPengendali == $this->USER_LOGIN_ID) { ?> 
                    <td><a style="color: #fff;" onclick="openAddFrame('main/loadUrlKontrak/kontrak/contracting_add_deliverable_bapp_edit?reqAidi=<?= $datadelivery->getField('DELIVERABLEID') ?>&reqConRekId=<?= $reqId ?>')" class="badge badge-primary"><span class="fa fa-edit"></span> Upload BAPP</a></td>
                  <?php } 
                  }?>
                  <td width="100px">
                    <?php
                  ...
                  <?php 
                  $today = new DateTime();
                  // Mendapatkan nilai tanggal dari objek $datadelivery
                  $tanggalDeliveryDari = new DateTime($datadelivery->getField('TANGGAL_DELIVERY_DARI'));
                  $tanggalDeliverySampai = new DateTime($datadelivery->getField('TANGGAL_DELIVERY_SAMPAI'));

                  // Mengecek apakah tanggal pengiriman berada dalam rentang hari ini
                  $blink = '';
                  $blinkStyle = '';
                  if ($tanggalDeliveryDari <= $today && $tanggalDeliverySampai >= $today && str_replace(' ','',$datadelivery->getField('STATUS')) == 'Proses') {
            ...
            <!-- <a href="kontrak/index/contracting_detail?reqId=<?= $reqId; ?>&reqProses=<?= $reqProses; ?>" class="<?= CLASS_BTN_DANGER ?>"> <?= BTN_KEMBALI ?> </a> -->
          </div>          
        </div>
      </div>
    </div>
