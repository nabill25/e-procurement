...
$reqPenyesuaianUpdated = $proses4->getField('CR_PENYESUAIAN_UPDATED_DATE') ? explode(' ',$proses4->getField('CR_PENYESUAIAN_UPDATED_DATE')) : '';
$reqPenyesuaianUpdatedDate = $reqPenyesuaianUpdated[0];
$reqPenyesuaianUpdatedDate2 = $reqPenyesuaianUpdated[1];

$textMonitoring->selectText(array("A.TYPE" => 'Penyesuaian'));
  ...
  <div class="col-md-9 col-sm-9">
    <div class="card">
      <div class="card-content collapse show border-info border-darken-2">
        <div class="card-body">
          <h4 class="mb-2">Penyesuaian Harga</h4>
      <?php
      if ($reqPenyesuaianAlasan == '')
      ...
      ?>
          <div class="row mb-1">
            <div class="col-md-12">
              ...
              <a class="<?= CLASS_BTN_DANGER ?>" onClick="openAdd('kontrak/loadUrl/kontrak/contracting_monitoring_harga_edit/?reqId=<?=$reqId?>');" style="color:#fff"> <span class="fa fa-angle-double-right" style="color:#fff"></span>  Lakukan Penyesuaian Harga ?</a>
            </div>
          </div>
      <?php
      } else
        ...
        echo '<div class="card mb-1 border-blue border-darken-1" style="padding: 5px 10px 0 10px; background-color: #fff3f3">
                <div class="row">
                  <div class="form-group col-md-12 mb-2">
                    <small>'.getFormattedDate($reqPenyesuaianUpdatedDate).' '.$reqPenyesuaianUpdatedDate2.'</small><br><b>Alasan Penyesuaian harga</b>: <i>'.$reqPenyesuaianAlasan.'</i>
                  </div>
                </div>
              </div>';
      ?>
          ...
          <div class="card mb-1 border-blue border-darken-1">
            <div class="card-content">
              <div class="p-1">
                <h5>Dokumen Penyesuaian
                <?php
                if ($reqContractingStatusKontrakId >= '2') { // Penyedia sudah approve ?>
                <small style="font-size:0.7em">
                  <a onclick="openAdd('main/loadUrlKontrak/kontrak/contracting_file_add?reqAidi=<?= $reqId ?>&reqProses=4&reqJenis=Penyesuaian Harga')" class="badge badge-primary pull-right mr-1 text-white"> <i class="fa fa-plus-circle"></i> Tambah Dokumen </a>
                </small>
                <?php
                } ?>
                </h5>
                <div class="table-responsive">
                  <table id="dokumenFileIdTable" class="border-double table mb-0 table-bordered mb-2" style="width: 100%">
                    <?= $this->libkontrak->getTableFile($reqId," AND A.FILE_JENIS = 'Penyesuaian Harga' ") ?>
                  </table>
                </div>
              </div>
            </div>
          ...
          <div class="form-actions">
            ...
            <a href="kontrak/index/contracting_persiapan_kontrak_edit?reqId=<?= $reqId ?>&back=contracting_monitoring_harga" class="<?= CLASS_BTN_PRIMARY ?> mr-1 mb-1 text-white"> <i class="fa fa-pencil"></i> Edit Data Kontrak </a>
            <?php
            } else { ?>
            <a href="kontrak/index/contracting_persiapan_kontrak_edit_legal?reqId=<?= $reqId ?>&back=contracting_monitoring_harga" class="<?= CLASS_BTN_PRIMARY ?> mr-1 mb-1 text-white"> <i class="fa fa-pencil"></i> Edit Data <?= $reqJenisKontrak ?> </a>
            <?php
            } ?>
