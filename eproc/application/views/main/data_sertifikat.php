        ...
        <div class="row row-treegrid">
        	<div class="col-md-12 col-treegrid">
            	<div class="area-konten-atas">
                	<div class="judul-halaman">Sertifikat Perusahaan
                    	<div class="info">
							<i class="fa fa-warning" aria-hidden="true"></i> Double-click untuk memilih sertifikat.  
                        </div>                    
                    </div>
                    
                </div>
                <div id="idpencarian" class="area-pencarian">
                    <label>Ketik nama sertifikat : <input class="" placeholder="" name="reqPencarian" aria-controls="example" type="search"></label>
                </div> 
				<div id="tableContainer" class="tableContainer tableContainer-treegrid">
                	<table id="treeSatker" class="easyui-treegrid" style="width:700px;height:300px"
                            data-options="
                                url: 'rekanan_sertifikat_json/get_data_sertifikat',
                                method: 'get',
                                idField: 'id',
                                treeField: 'text'
                            ">
                        <thead>
                            <tr>
                                ...
                                <th data-options="field:'NOMOR'" >Nomor</th>
    ...
    $(document).ready( function () {
		...
		$('input[name=reqPencarian]').keypress(function( event ) {
			if ( event.which == 13 ) {
				...
				var urlApp = 'rekanan_sertifikat_json/get_data_sertifikat/?reqSearch='+ value;
				$('#treeSatker').treegrid(
				{
					url: urlApp
				});	
        ...
        $('#treeSatker').treegrid({
              onDblClickRow: function(param){
				 ...
				 if($reqMode == "KUALIFIKASI")
				 	echo "top.addKualifikasiSertifikatLain(param.id, param.NAMA); top.closePopup();";
				 else
				 	echo "top.tambahSertifikat(param.id, param.NAMA);";
				 ?>	
				  }
			});
		});
