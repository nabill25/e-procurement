...
$getTahun = isset($_GET['tahun']) ? $_GET['tahun'] : 'all';
  ...
  if ($getTahun != 'all'){
    ...
    $dash1->getCountPlanning($unitkerja,$this->USER_LOGIN_ID, $getTahun);
    $dash1->firstRow();
  } else {
    $tahun = '';
    $dash1->getCountPlanning($unitkerja,$this->USER_LOGIN_ID);
    $dash1->firstRow();
  }
  ...
  $dash2->getCountPlanning2($unitkerja,$this->USER_LOGIN_ID, $getTahun); 
...
<div class="row">
  <div class="form-group col-md-12">
    ...
    <select class="form-control" id="setyear" onChange="return window.location = $(this).val()">
      <?php
      $selected = '';
      $url = base_url('main/index/dashboardunit?tahun=');
      $kurangdari = date('Y') - 5;
...
<div class="row">
   <div class="col-xl-12 col-lg-12 col-md-12 animated zoomIn">
      <div class="card">
        <div class="card-body">
          <h3 class="mt-1 success darken-1 text-center">Timeline Perencanaan <?= $tahun ?></h3>
          <div id="project-info" class="card-body row">

            <div class="project-info-count col-lg-1 col-md-12"></div>
            <div class="project-info-count col-lg-2 col-md-12">
               <div class="project-info-icon">
                  <h2><?= numberToSimbol($total_inputan) ?></h2>
                  <div class="project-info-sub-icon">
                     <span class="fa fa-edit"></span>
                  </div>
               </div>
               <div class="project-info-text pt-1">
                  <h2><?= numberToSimbol($perkiraan_biaya_harga_inputan) ?></h2>
                  <h7>Input Usulan Kebutuhan</h7>
               </div>
            </div>
            <div class="project-info-count col-lg-2 col-md-12">
               <div class="project-info-icon">
                  <h2><?= numberToSimbol($total_verifikator) ?></h2>
                  <div class="project-info-sub-icon">
                     <span class="fa fa-tags"></span>
                  </div>
               </div>
               <div class="project-info-text pt-1">
                  <h2><?= numberToSimbol($perkiraan_biaya_harga_verifikator) ?></h2>
                  <h7>Verifikator</h7>
               </div>
            </div>
            <div class="project-info-count col-lg-2 col-md-12">
               <div class="project-info-icon">
                  <h2><?= numberToSimbol($total_validator) ?></h2>
                  <div class="project-info-sub-icon">
                     <span class="fa fa-user-circle"></span>
                  </div>
               </div>
               <div class="project-info-text pt-1">
                  <h2><?= numberToSimbol($perkiraan_biaya_harga_validator) ?></h2>
                  <h7>Validator</h7>
               </div>
            </div>
            <div class="project-info-count col-lg-2 col-md-12">
               <div class="project-info-icon">
                  <h2><?= numberToSimbol($total_approval) ?></h2>
                  <div class="project-info-sub-icon">
                     <span class="fa fa-font-awesome"></span>
                  </div>
               </div>
               <div class="project-info-text pt-1">
                  <h2><?= numberToSimbol($perkiraan_biaya_harga_approval) ?></h2>
                  <h7>Approval</h7>
               </div>
            </div>
            <div class="project-info-count col-lg-2 col-md-12">
               <div class="project-info-icon">
                  <h2><?= numberToSimbol($total_rencana_pengadaan) ?></h2>
                  <div class="project-info-sub-icon">
                     <span class="fa fa-check-circle"></span>
                  </div>
               </div>
               <div class="project-info-text pt-1">
                  <h2><?= numberToSimbol($hps); ?></h2>
                  <h7>Rencana Pengadaan</h7> 
               </div>
            </div>
         </div>
        </div>
...
<div class="row">
  ...
  <div class="col-xl-4 col-lg-4 col-md-12 animated zoomIn">
      <div class="card">
      ...
      <div class="card-content">
        <div class="card-body" style="padding: .7em">
          <span class="text-center">
            <h3 class="mt-1 success darken-1">Lama Proses Perencanaan <br><small><?= $tahun ?></small></h3>
          </span> 
          <table id="prosesDash" class="border-double table mb-0 table-bordered" style="width: 100%">
            <thead>
              <tr>
                ...
                <th>Nama Paket</th>
                <th>Status</th>
                <th align="center">Lama Proses</th>
              </tr>
            </thead>
            <tbody>
              <?php 
              ...
              while($dash2->nextRow()) 
              { 
                $a = $dash2->getField('LAMA_PROSES');
                if ($a == '00:00:00') {
                  $lamaproses = '-';
                } else {
                  $b = str_replace('days','Hari', $a);
                  $c = explode(' Hari ',$b);
                  $d = explode(':',$c[1]);

                  $z1 = $c[0].' Hari ';
                  $z2 = $d[0].' Jam '.$d[1].' Menit '.$d[2].' Detik';
              ...
              ?>
                <tr>
                  ...
                  <td> <?= $dash2->getField('NAMA') ?></td>
                  <td> <?= $dash2->getField('STATUS') ?></td>
                  <td> <?= $lamaproses ?></td>
                </tr>
              <?php 
              $no++;
