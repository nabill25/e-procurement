...
$libapi = new libapi(); 
$url = 'http://117.53.144.136:8070/v01/mataanggaran';
$date = date('Y');
...
else
{
  ...
  $reqTahunAnggaran = $permohonan_paket_analisa->getField("TAHUN_ANGGARAN");
            ...
            <div class="card-content">
              <div class="p-1">
                <div class="alert bg-primary alert-icon-left alert-arrow-left alert-dismissible mb-2" role="alert">
                  ...
                  <a onclick="createRowNotaDinas()"> <span class="fa fa-plus   text-white font-medium-2 icon-line-height" data-toggle="tooltip" data-placement="top" title="Tambah"></span> </a> 
                </div> 
                <div class="table-responsive">
                  <table class="border-double table mb-0">
                    <thead>
                    ...
                    <tbody id="tbodyPermohonanPaketAnalisaFile">
                        <?php
                        $permohonan_paket_analisa_file = new PermohonanPaketAnalisaFile();
                        $no = 1;
                        $permohonan_paket_analisa_file->selectByParams(array("PERMOHONAN_PAKET_ANALISA_ID" => coalesce($usulanId, 0)));
                        //echo $permohonan_paket_analisa_file->query;exit;
                        while($permohonan_paket_analisa_file->nextRow())
                        {
                        ?>
                         <tr id="tr1<?=$no?>">
                             <td>
                                <input class="form-control easyui-validatebox span4" type="hidden"  name="reqPermohonanPaketAnalisaFileId[]" id="reqPermohonanPaketAnalisaFileId<?=$no?>" value="<?=$permohonan_paket_analisa_file->getField("PERMOHONAN_PAKET_FILE_ID")?>"/>
                                
                                <input class="form-control easyui-validatebox span4" type="text" required name="reqJudul[]" id="reqJudul<?=$no?>" value="<?=$permohonan_paket_analisa_file->getField("JUDUL")?>" <?php if ($no <= 4) {?> readonly <?php } ?>/>
                             </td>
                             <td>
                                <input type="file" name="reqLinkFile[]" id="reqLinkFile<?=$no?>" size="30" class="easyui-validatebox" validType="fileType['pdf','zip']"/>
                                <input type="hidden" name="reqLinkFileTemp[]" id="reqLinkFileTemp<?=$no?>" value="<?=$permohonan_paket_analisa_file->getField("PATH_FILE")?>">
                                <input type="hidden" name="reqLinkFileTempTipe[]" id="reqLinkFileTempTipe<?=$no?>" value="<?=$permohonan_paket_analisa_file->getField("TIPE")?>">
                                <input type="hidden" name="reqLinkFileTempUkuran[]" id="reqLinkFileTempUkuran<?=$no?>" value="<?=$permohonan_paket_analisa_file->getField("UKURAN")?>">
                                <input type="hidden" name="reqUrut[]" id="reqUrut<?=$no?>" value="<?=$permohonan_paket_analisa_file->getField("URUT")?>">
                                <br>
                             </td>
                             <td>
                                <?php 
                                if ($permohonan_paket_analisa_file->getField("PATH_FILE")) {
                                   echo '<a href="uploads/permohonan_paket/'.$permohonan_paket_analisa_file->getField("PATH_FILE").'" target="_blank"><i class="fa fa-download"></i></a>';
                                 } ?> 
                                 ...
                                 {
                                 ?>
                                    <a onClick="deleteDataTable('permohonan_paket_file_json/delete/', '<?=$permohonan_paket_analisa_file->getField("PERMOHONAN_PAKET_FILE_ID")?>', 'tr1<?=$no?>')"
                        ...
                        if($no == 1)
                        {
                            $arrDokumenWajib = array("Draft KAK / Spesifikasi","Draft Kontrak","Draft RAB");
                            for($i=0; $i<count($arrDokumenWajib);$i++)
                            {
                        ?>
                                 <tr id="tr1<?=$no?>">
                                     <td>
                                            <input class="form-control easyui-validatebox" type="hidden"  name="reqPermohonanPaketAnalisaFileId[]" id="reqPermohonanPaketAnalisaFileId<?=$no?>" value="" />
                                     ...
                                     <td>
                                            ...
                                            if ($permohonan_paket_analisa_file->getField("PATH_FILE")) {
                                            ?>
                                            <br>file : <?=$permohonan_paket_analisa_file->getField("PATH_FILE")?>
                                            <?php
                                             } ?>
                                     </td>
                                     <td>
