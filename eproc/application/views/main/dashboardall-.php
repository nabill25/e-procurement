...
-->
    ...
    $getUser->selectByParams(array("USER_LOGIN_ID" => 572));
    $getUser->firstRow();
    $userVPPengadaan1 = $getUser->getField("USER_NAMA");
    $getDataChartGauge = new Dashpaket();
    if ($getTahun != 'all'){
      $getDataChartGauge->getDashboardGauge($unitkerja,572,$getTahun);
      ...
      $getDataChartGauge->getDashboardGauge($unitkerja,572);
      $getDataChartGauge->firstRow();
      $totalPaketGauge = $getDataChartGauge->getField('TOTAL_PAKET') ?: 0;
      $totalPaketProsesGauge = $getDataChartGauge->getField('TOTAL_PAKET_PROSES') ?: 0;
    }
    ...
    $(function() {
      ...
      $(window).on("load", function(){
          document.getElementById("gaugeArea").onclick = function (evt) {
            var tahun   = $('#tahun').val();
            var url = 'main/loadUrl/main/dashboard_detail_gauge_new/?tahun='+tahun+'&uki=<?= $unitkerja ?>&type=572';
            openAdd(url);
          };
      });
    });
    ...
    // Laporan Berdasarkan Beban Kerja Paket
    $getUser = new UserLogin();
    $getUser->selectByParams(array("USER_LOGIN_ID" => 571));
    $getUser->firstRow();
    $userVPPengadaan2 = $getUser->getField("USER_NAMA");
    $getDataChartGauge = new Dashpaket();
    if ($getTahun != 'all'){
      $getDataChartGauge->getDashboardGauge($unitkerja,571,$getTahun);
      $getDataChartGauge->firstRow();
      $totalPaketGauge = $getDataChartGauge->getField('TOTAL_PAKET') ?: 0;
      $totalPaketProsesGauge = $getDataChartGauge->getField('TOTAL_PAKET_PROSES') ?: 0;
    } else {
      $getDataChartGauge->getDashboardGauge($unitkerja,571);
      $getDataChartGauge->firstRow();
      $totalPaketGauge = $getDataChartGauge->getField('TOTAL_PAKET') ?: 0;
      $totalPaketProsesGauge = $getDataChartGauge->getField('TOTAL_PAKET_PROSES') ?: 0;
    }
    ...
    $(function() {
      ...
      document.getElementById("gaugeArea2").onclick = function (evt) {
        var tahun   = $('#tahun').val();
        var url = 'main/loadUrl/main/dashboard_detail_gauge_new/?tahun='+tahun+'&uki=<?= $unitkerja ?>&type=571';
        openAdd(url);
      };
    });
    </script>
    ...
    <div class="col-md-6">
        <div class="card">
          <div class="card-header card-head-inverse bg-primary">
            <h4 class="card-title text-white" style="font-size:.9em !important">Laporan Berdasarkan Beban Kerja P2 = <?= $totalPaketGauge  ?> Paket <?= $tahun ?> </h4>
            <a class="heading-elements-toggle"><i class="fa fa-ellipsis-v font-medium-3"></i></a>
            <div class="heading-elements">
                <ul class="list-inline mb-0">
                  <li><a data-action="expand"><i class="ft-maximize"></i></a></li>
          ...
          <div class="card-content collapse show text-center border-info border-darken-2" style="padding-bottom: 33px">
            ...
            <small><b>Jumlah Paket Yang Sedang Diproses P2 (<?= $userVPPengadaan2 ?>) </b></small>
          </div>
        </div>
    </div>
