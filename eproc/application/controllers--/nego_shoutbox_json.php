...
class nego_shoutbox_json extends CI_Controller {
	...
	function nego_shoutbox() 
	{
		...
		/* validasi */
		if($this->USER_TYPE_ID == "3"  || $this->USER_TYPE_ID  == "7" || $this->USER_TYPE_ID  == "11")
		{}
		elseif($this->USER_TYPE_ID == "6")
		{
		  ...
		  switch($_GET['action']) {
			case 'add':
				...
				$_POST['message2'] = htmlentities($_POST['message2']);

				if ($_POST['message'] != '') {
					$pesan = $_POST['message'] = htmlentities($_POST['message']);
				}

				if ($_POST['message2'] != '') {
					$pesan = $_POST['message2'] = htmlentities('Perubahan harga menjadi '.$_POST['message2']);
				}
				...
				$renameFile = 'chat_nego_'.md5(date("dmYHis").$reqLinkFile['name'].$this->ID).".".getExtension($reqLinkFile['name']);
				...
				$nego_shoutbox->setField("FILE", $insertLinkFile);
				...
				$nego_shoutbox->setField("REKANAN_ID", $reqRekananId); 
				...
				$data['response'] = 'Alhamdulillah berjalan';
			...
			case 'view':
			  ...
			  if(!$_GET['time'])
				$_GET['time'] = 0;
			  $nego_shoutbox = new NegoShoutbox();
			  $nego_shoutbox->selectByParams(array("PAKET_PENAWARAN_ID" => $reqPaketPenawaranId, "REKANAN_ID" => $_GET['rekananid']));
			  $message_html = '';
			  while($nego_shoutbox->nextRow())
			  {
					$file = $nego_shoutbox->getField('FILE');
			  		if (file_exists($FILE_DIR.$file) && !$file) 
			  		{ $filenya=''; 
						$filenya = $nego_shoutbox->getField("PESAN");
			  		} else {
						$filenya = $nego_shoutbox->getField("PESAN").'<a href="'.$FILE_DIR.$file.'" target="_blank">
									<small class="badge badge-warning"><span class="fa fa-download"></span> download file</small>
									</a>';
					} 
			  	$message_html .= '<li class="left clearfix">
                            		<div class="chat-body clearfix">
                            		<div class="triangle-isosceles left">
                            		<div class="header"><strong class="primary-font">'.$nego_shoutbox->getField("NAMA").'</strong> <small class="pull-right text-muted">
                            		<i class="fa fa-clock-o" aria-hidden="true"></i>'.$nego_shoutbox->getField("WAKTU").'</small>
                            		</div>
                            		<p> 
                            		'.$filenya.'
                            		</p>
