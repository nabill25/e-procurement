        ...
        } else 
        {
			...
			if($submitSimpan == "Simpan")
			{
				...
				for($i=0;$i<count($reqPaketPenawaranId);$i++)
				{
					...
					// Insert to Logs
					$filepath = 'logs/Penawaran-log_' .$reqId. '.txt'; 
			      	$handle = fopen($filepath, "a+");
			      	$text   = "";
		      		$text   .= "TIME:".date('h:i:s')." ### DATE:".date('Y-m-d')." ### USERID:". $this->ID." ### IP:". $this->getIP()." ### STATEMENT: ".$this->REKANAN." input Nilai Penawaran sebesar ".$reqJumlah[$i]."
		-----------------------------------------------------------------------------------------";
					$arr = array('<br>');
