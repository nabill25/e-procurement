...
class dokumen_pengadaan_upload_rekanan extends CI_Controller {
	...
	function upload_validasi() 
	{
		...
		if($paketInfo->sistem_sampul == "2")
		{
			if($reqJenisDokumen == "PENAWARAN_HARGA")
				$reqToken= sha1("2_".$paketInfo->publish_ba_penawaran.$paketInfo->pr_group_number.$paket_rekanan->getField("TANGGAL_DAFTAR_ENKRIPSI").$paket_rekanan->getField("KODE_REKANAN").$reqId.$paketInfo->user_login_id).PASS_END_PENAWARAN;			
			else
				$reqToken= sha1("1_".$paketInfo->publish_ba_penawaran.$paketInfo->pr_group_number.$paket_rekanan->getField("TANGGAL_DAFTAR_ENKRIPSI").$paket_rekanan->getField("KODE_REKANAN").$reqId.$paketInfo->user_login_id).PASS_END_PENAWARAN;
		}
		else	
			$reqToken= sha1($paketInfo->publish_ba_penawaran.$paketInfo->pr_group_number.$paket_rekanan->getField("TANGGAL_DAFTAR_ENKRIPSI").$paket_rekanan->getField("KODE_REKANAN").$reqId.$paketInfo->user_login_id).PASS_END_PENAWARAN;
		...
		// $arrDokumenPenawaran 	 	 = array(0, 11, 6,  11, 6,  10, 6,  11, 11, 0, 0, 11, 6,  11, 6, 5);
	...
	function email() 
	{
		...
		if($paketInfo->sistem_sampul == "2")
		{
			...
			$reqFilename  = FILENAME_PENAWARAN."1_".$paketInfo->publish_ba_penawaran.$paketInfo->pr_group_number."-".$paket_rekanan->getField("KODE_REKANAN");
			$reqToken	  = sha1("1_".$paketInfo->publish_ba_penawaran.$paketInfo->pr_group_number.$paket_rekanan->getField("TANGGAL_DAFTAR_ENKRIPSI").$paket_rekanan->getField("KODE_REKANAN").$reqId.$paketInfo->user_login_id).PASS_END_PENAWARAN;
			...
			$reqFilename2 = FILENAME_PENAWARAN."2_".$paketInfo->publish_ba_penawaran.$paketInfo->pr_group_number."-".$paket_rekanan->getField("KODE_REKANAN");
			$reqToken2	  = sha1("2_".$paketInfo->publish_ba_penawaran.$paketInfo->pr_group_number.$paket_rekanan->getField("TANGGAL_DAFTAR_ENKRIPSI").$paket_rekanan->getField("KODE_REKANAN").$reqId.$paketInfo->user_login_id).PASS_END_PENAWARAN;			
			...
			$body = file_get_contents(base_url()."main/loadUrl/email/".$link_email."/".$reqId."/".$this->ID);
			$mail->MsgHTML($body);
			if($mail->Send())
			{
				unlink($reqFile);
		...
		{
			$reqFilename= FILENAME_PENAWARAN.$paketInfo->publish_ba_penawaran.$paketInfo->pr_group_number."-".$paket_rekanan->getField("KODE_REKANAN");
			$reqToken= sha1($paketInfo->publish_ba_penawaran.$paketInfo->pr_group_number.$paket_rekanan->getField("TANGGAL_DAFTAR_ENKRIPSI").$paket_rekanan->getField("KODE_REKANAN").$reqId.$paketInfo->user_login_id).PASS_END_PENAWARAN;
