      ...
      <div class="card-content collapse show border-info border-darken-2">
        <div class="card-body">
          ...
          <small style="font-size:0.7em">
            <a onclick="openAdd('main/loadUrlKontrak/kontrak/contracting_add_payment2?reqAidi=<?= $reqId ?>-<?= $reqMetodePembayaran ?>')" class="badge badge-primary pull-right mr-1 text-white mb-1"> <i class="fa fa-plus-circle"></i> Tambah Pembarayan </a>
          </small>
          <div class="form-actions table-responsive">
            <table id="tabletermin" class="border-double table mb-0 table-bordered mb-2" style="width: 100%">
              ...
              if ($datapayment->countRow() > 0) {
                ...
                <tr>
                  ...
                  <?php
                  if ($this->LEGAL != '1' && $this->USER_TYPE_ID != '20') { // update status oleh peng. kontrak bukan legal, pemeriksa kontrak ?>
                  <td style="padding:5px 10px !important; text-align: center">
                    <a style="color: #fff;" onclick="openAdd('main/loadUrlKontrak/kontrak/contracting_add_payment_edit2?reqAidi=<?= $datapayment->getField('PAYMENTID') ?>-<?= $reqMetodePembayaran ?>&reqConRekId=<?= $reqId ?>')" class="badge badge-info"><i class="fa fa-edit"></i></a>
                    <a style="color: #fff;" onClick="delPayment('<?= $datapayment->getField('PAYMENTID') ?>','Hapus data <?= $datapayment->getField('PAY_KETERANGAN') ?>?')" class="badge badge-danger"><i class="fa fa-trash"></i></a>
                  </td>
                  <?php
                  } else { echo '<td>-</td>'; }?>
                </tr>
            ...
            <?php
            if ($this->LEGAL != '1' && $this->USER_TYPE_ID != '20') { // Pengelola Kontrak bukan Legal, pemeriksa kontrak ?>
            <?php
             if ($datapayment->countRow() > 0 && $tombolTutup == $datapayment->countRow()) {
