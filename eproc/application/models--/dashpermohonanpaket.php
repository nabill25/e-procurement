 ...
 */
  ...
  class DashPermohonanPaket extends Entity{
  ...
  **/
  ...
  function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order="ORDER BY A.PERMOHONAN_PAKET_ID DESC")
	{
		$str = "SELECT A.* FROM (
						SELECT A.PERMOHONAN_PAKET_ID, D.PAKET_ID, A.USER_LOGIN_ID, A.UNIT_KERJA_ID, A.POSTING, A.PENGADAANLANGSUNG, A.TAHUN_ANGGARAN,
				   		A.NOTA_DINAS, A.NAMA, A.KETERANGAN, A.NO_PPA, A.TANGGAL, date_part('month'::text, A.RENCANA_PENGADAAN) AS BULAN_RENCANA_PENGADAAN, date_part('year'::text, A.RENCANA_PENGADAAN) AS TAHUN_RENCANA_PENGADAAN, A.PENGADAANLANGSUNG, C.NAMA UNIT_KERJA, E.USER_NAMA USER_LOGIN, F.USER_TYPE_ID,
	                    CASE WHEN D.PAKET_ID IS NULL THEN 0 ELSE 1 END STATUS,
						CASE WHEN (A.PIC IS NULL) THEN 0 ELSE 1 END STATUS_PIC,
						...
						END
						STATUS_KETERANGAN,
						MD5(A.PERMOHONAN_PAKET_ID || 'EPROC') PERMOHONAN_PAKET_ID_ENCRYPT, A.NILAI, N.HARGA_NEGOSIASI, N.PEMENANG, A.LAST_CREATE_USER, E.DEPARTMENT,A.ALASAN_TOLAK, F.NIPP, F.NAMA NAMA_PIC,
						...
						(SELECT '1' total from paket aa where aa.permohonan_paket_id=A.permohonan_paket_id
												) TOTAL_REALISASI,
						(SELECT X.CONTRACTINGREKANANID FROM CONTRACTING_REKANAN X WHERE D.PAKET_ID = X.PAKET_ID ) STATUS_KONTRAK
	          ...
	          LEFT JOIN PAKET D ON A.PERMOHONAN_PAKET_ID = D.PERMOHONAN_PAKET_ID
						...
						left join view_dash_efesiensi N on D.PAKET_ID = N.PAKET_ID
  ...
  function sumHpsByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order="ORDER BY A.PERMOHONAN_PAKET_ID DESC")
	{
		$str = "SELECT SUM(A.PERKIRAAN_BIAYA_HARGA) TOTAL_HPS FROM (
							SELECT A.* FROM 
							( 
								SELECT A.PERMOHONAN_PAKET_ID, A.USER_LOGIN_ID, A.UNIT_KERJA_ID, A.POSTING, A.PENGADAANLANGSUNG, A.TAHUN_ANGGARAN,
						   		A.NOTA_DINAS, A.NAMA, A.KETERANGAN, A.NO_PPA, A.TANGGAL, date_part('month'::text, A.RENCANA_PENGADAAN) AS BULAN_RENCANA_PENGADAAN, date_part('year'::text, A.RENCANA_PENGADAAN) AS TAHUN_RENCANA_PENGADAAN, A.PENGADAANLANGSUNG, C.NAMA UNIT_KERJA, E.USER_NAMA USER_LOGIN,
			                    CASE WHEN D.PAKET_ID IS NULL THEN 0 ELSE 1 END STATUS,
								CASE WHEN (A.PIC IS NULL) THEN 0 ELSE 1 END STATUS_PIC,
								CASE WHEN A.POSTING = '1' THEN
								CASE WHEN D.PAKET_ID IS NULL THEN 'Paket belum diproses' ELSE '<a onclick=\"top.location.href = ''main/index/paket_detil/?reqId=' || D.PAKET_ID || '''\">Detil Paket</a>' END
								...
								END
								STATUS_KETERANGAN,
								MD5(A.PERMOHONAN_PAKET_ID || 'EPROC') PERMOHONAN_PAKET_ID_ENCRYPT, A.NILAI, N.HARGA_NEGOSIASI, A.LAST_CREATE_USER, E.DEPARTMENT,A.ALASAN_TOLAK, F.NIPP, F.NAMA NAMA_PIC,
									G.APPROVAL, G.PUBLISH, G.NAMA_KEBUTUHAN, G.PERMOHONAN_PAKET_ANALISA_ID,
									A.PERKIRAAN_BIAYA_HARGA, E.VP_PENGADAAN, E.ADMIN_RUP, A.BUDGET_AWAL, A.BUDGET_TERPAKAI, A.BUDGET_AKHIR,
								(SELECT '1' total from paket aa where aa.permohonan_paket_id=A.permohonan_paket_id
														) TOTAL_REALISASI
								FROM PERMOHONAN_PAKET A
								LEFT JOIN USER_LOGIN E ON A.USER_LOGIN_ID = E.USER_LOGIN_ID
  ...
  function sumHargaNegoByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order="ORDER BY A.PERMOHONAN_PAKET_ID DESC")
	{
		$str = "SELECT SUM(A.HARGA_NEGOSIASI) TOTAL_HPS FROM (
							SELECT A.* FROM 
							( 
								SELECT A.PERMOHONAN_PAKET_ID, A.USER_LOGIN_ID, A.UNIT_KERJA_ID, A.POSTING, A.PENGADAANLANGSUNG, A.TAHUN_ANGGARAN,
						   		A.NOTA_DINAS, A.NAMA, A.KETERANGAN, A.NO_PPA, A.TANGGAL, date_part('month'::text, A.RENCANA_PENGADAAN) AS BULAN_RENCANA_PENGADAAN, date_part('year'::text, A.RENCANA_PENGADAAN) AS TAHUN_RENCANA_PENGADAAN, A.PENGADAANLANGSUNG, C.NAMA UNIT_KERJA, E.USER_NAMA USER_LOGIN,
  ...
  function selectMonth($paramsArray=array(),$limit=-1,$from=-1, $statement='',$order="ORDER BY MONTH_ID ASC")
	...
	{
		$str = "  SELECT PERMOHONAN_PAKET_ID, PERMOHONAN_PAKET_ANALISA_ID, USER_LOGIN_ID, A.UNIT_KERJA_ID, TANGGAL, NOTA_DINAS, PENGADAANLANGSUNG, A.TAHUN_ANGGARAN,
					     A.NAMA, KETERANGAN, NO_PPA, LAST_CREATE_USER, LAST_CREATE_DATE, B.NAMA UNIT_KERJA, A.NILAI, A.BUDGET_AWAL, A.BUDGET_TERPAKAI, A.BUDGET_AKHIR, KODE_RUP, KODE_PR
				  FROM PERMOHONAN_PAKET A
    ...
    function insertAnalisa()
	{
		...
		$str = "
			...
			INSERT INTO PERMOHONAN_PAKET_ANALISA (
			   PERMOHONAN_PAKET_ANALISA_ID, TAHUN_ANGGARAN, CREATED_BY, CREATED_DATE,APPROVAL,PUBLISH,PERMOHONAN_PAKET_ANALISA_KATEGORI_ID,NOTE,SUMBER_DANA_KETERANGAN)
  			 	VALUES (
    ...
    function insertPermohonan()
	{
		...
		$str = "
			...
			INSERT INTO PERMOHONAN_PAKET (
			   ...
			   ANGGARAN, JENIS_BARANG_JASA, PERKIRAAN_BIAYA_HARGA, NILAI, WAKTU_PENGGUNA_BARANGJASA, RENCANA_PENGADAAN, LAST_CREATE_USER, LAST_CREATE_DATE, CARA_PENGADAAN, USER_LOGIN_ID, UNIT_KERJA_ID)
  ...
  function selectByParamsUsulanView($paramsArray=array(),$limit=-1,$from=-1, $statement='', $order=" ORDER BY PERMOHONAN_PAKET_ANALISA_ID DESC")
	{
    ...
    function selectByParamsUsulan($paramsArray=array(),$limit=-1,$from=-1, $statement='', $order=" ORDER BY A.PERMOHONAN_PAKET_ANALISA_ID DESC")
	{
		$str = "SELECT A.* FROM
					...
					ELSE '' END CARA_PENGADAAN_STR, I.NAMA, A.PERMOHONAN_PAKET_ANALISA_KATEGORI_ID AS KATEGORI, K.NAMA KATEGORI_STR, A.PERMOHONAN_PAKET_ANALISA_JENIS_BELANJA_ID AS JENIS_BELANJA, L.NAMA JENIS_BELANJA_STR, I.PERMOHONAN_PAKET_ID,
    ...
    function getCountByParamsUsulan($paramsArray=array(), $statement='')
	{
		$str = "SELECT COUNT(A.PERMOHONAN_PAKET_ANALISA_ID) AS ROWCOUNT FROM
				(
					SELECT A.* FROM
					(
						SELECT A.*, I.RENCANA_PENGADAAN, E.ADMIN_RUP
						FROM PERMOHONAN_PAKET_ANALISA A
						LEFT JOIN USER_LOGIN E ON A.CREATED_BY = E.USER_LOGIN_ID
	                    LEFT JOIN V_OAUTH_USER B ON E.NIP = B.NIPP
	                    LEFT JOIN KOMODITAS C ON A.KOMODITAS_ID = C.KOMODITAS_ID
